...
//! retargeted at PVM via the `vos_pvm` interpreter and ECALL host-call
//! protocol.
//!
...
//!
//! ```ignore
//! use vos_pvm_proof::{trace_blob, prove_mobile, verify};
//!
...
//!   `vos_pvm`'s semantics so trace fill matches the interpreter
//!   byte-for-byte.
...
// generated impls (which target `::vos_pvm_proof::AirColumn`) resolve correctly.
...
#[cfg(feature = "prover")]
pub use prove::{
    canonical_profile_for, canonical_profile_for_bounds, program_commitment_for_profile, prove,
    prove_canonical, prove_chain, prove_mobile, prove_with_config,
};
