    ...
    for (row, step) in side_note.steps.iter().enumerate() {
        ...
        *side_note.program_memory_counts.entry(step.pc).or_insert(0) += 2;
        ...
        let (mut val_b, mut val_d) = match step.opcode.category() {
            vos_pvm::instruction::InstructionCategory::ThreeReg => {
                (step.regs_before[step.reg_a], step.regs_before[step.reg_b])
            }
            vos_pvm::instruction::InstructionCategory::TwoRegOneImm
                if flags.is_rotate_r_imm_alt =>
            {
                // Rotate-imm-alt swap.
                (step.imm, step.regs_before[step.reg_b])
            }
            vos_pvm::instruction::InstructionCategory::TwoRegOneImm if flags.is_set_gt => {
                // SetGt swap: val_b ← imm, val_d ← regs[rb], so the SetLt
                // comparison computes (imm < reg) = (reg > imm) = the GT result.
                (step.imm, step.regs_before[step.reg_b])
            }
            vos_pvm::instruction::InstructionCategory::TwoRegOneImm if flags.is_cmov_imm => {
            ...
            vos_pvm::instruction::InstructionCategory::TwoReg => (0, step.regs_before[step.reg_b]),
            vos_pvm::instruction::InstructionCategory::OneRegImmOffset => {
                // BranchEqImm/NeImm/LtImm/etc: compare regs[ra] vs imm
                (step.regs_before[step.reg_a], step.imm)
            }
            _ if uses_immediate(step.opcode) => (0, step.imm),
        ...
        if flags.is_add {
            ...
            let (a, b) = if flags.is_neg_add {
                (val_d_bytes, val_b_bytes)
            } else {
                (val_b_bytes, val_d_bytes)
            };
            let mut c: u16 = 1;
            for i in 0..carry_limbs {
        ...
        if flags.is_bitwise {
            ...
            side_note
                .bitwise_entries
                .push(crate::side_note::BitwiseEntry {
                    val_b,
                    val_d,
                    result,
                    and_result,
        ...
        if flags.is_compare || flags.is_branch {
            ...
            side_note
                .compare_entries
                .push(crate::side_note::CompareEntry {
                    val_b,
                    val_d,
                    cmp_lt_flag,
                    cmp_sub_result,
        ...
        let sign_src_b: u8 = if flags.is_32bit {
            val_b_bytes[3]
        } else {
            val_b_bytes[7]
        };
        let sign_src_d: u8 = if flags.is_32bit {
            val_d_bytes[3]
        } else {
            val_d_bytes[7]
        };
        ...
        trace.fill_columns(
            row,
            flags.is_sub && !flags.is_neg_add,
            Column::IsSubNotNegaddH,
        );
        trace.fill_columns(row, flags.is_sub && flags.is_neg_add, Column::IsSubNegaddH);
        trace.fill_columns(
            row,
            flags.is_sub && !flags.is_neg_add && is_64bit_b,
            Column::IsSub64NotNegaddH,
        );
        trace.fill_columns(
            row,
            flags.is_sub && flags.is_neg_add && is_64bit_b,
            Column::IsSub64NegaddH,
        );
        ...
        let is_halt_jump_ind = flags.is_jump_ind
            && step.exit
            && (val_b.wrapping_add(step.imm) as u32 == vos_pvm::PVM_HALT_ADDR as u32);
        ...
        let is_halt_load_imm_jump_ind = flags.is_load_imm_jump_ind
            && step.exit
            && (val_d.wrapping_add(step.imm_y) as u32 == vos_pvm::PVM_HALT_ADDR as u32);
        ...
        if flags.is_div_rem {
            ...
            if divisor == 0 {
                ...
                if flags.is_div_s && !flags.is_32bit {
                    let sa = (val_b >> 63) & 1;
                    let sd = (val_d >> 63) & 1;
                    let term_d = if (div_quotient[7] >> 7) & 1 == 1 {
                        val_d
                    } else {
                        0
                    };
                    ...
                    for i in 0..WORD_SIZE {
                        ...
                        //     − (i==0 ? sa : 0)
                        let s = term_d_bytes[i] as i32 + term_q_bytes[i] as i32 + carry + extra_rhs
                            - extra_lhs;
        ...
        if flags.is_div_rem {
            let mut c: u16 = 0;
            for i in 0..WORD_SIZE {
                let s = val_d_bytes[i] as u16 + (255u16 - div_remainder[i] as u16) + c;
        ...
        if flags.is_div_rem {
            let sign_d_p30: u8 = if flags.is_32bit {
                (val_d_bytes[3] >> 7) & 1
            } else {
                (val_d_bytes[7] >> 7) & 1
            };
            let sign_r_p30: u8 = if flags.is_32bit {
                (div_remainder[3] >> 7) & 1
            } else {
                (div_remainder[7] >> 7) & 1
            };
        ...
        // nibble lookups.
        let sign_src_q: u8 = if flags.is_32bit {
            div_quotient[3]
        } else {
            div_quotient[7]
        };
        let sign_src_r: u8 = if flags.is_32bit {
            div_remainder[3]
        } else {
            div_remainder[7]
        };
        ...
        if flags.is_div_rem {
            ...
            side_note
                .divrem_entries
                .push(crate::side_note::DivRemEntry {
                    val_b,
                    val_d,
        ...
        *side_note
            .bitwise_and_counts
            .entry((load_sign_hi, 8))
            .or_insert(0) += 1;
        *side_note
            .bitwise_and_counts
            .entry((load_sign_lo, 0xF))
            .or_insert(0) += 1;
        ...
        // div_by_zero, val_d_is_zero, flags.div_rem_op) are in scope.
        {
            ...
            let is_compare_b = flags.is_set_lt_u
                || flags.is_set_lt_s
                || flags.is_cmov_iz
                || flags.is_cmov_nz
                || flags.is_min_s
            ...
            trace.fill_columns(row, flags.is_cmov_nz && !vd_zero_b, Column::IsCmovNzNotVdzH);
            ...
            let gate_div_v =
                ((op_i32 - 2) * (op_i32 - 3)).rem_euclid(stwo::core::fields::m31::P as i32) as u32;
            let gate_rem_v =
                (op_i32 * (op_i32 - 1)).rem_euclid(stwo::core::fields::m31::P as i32) as u32;
            trace.fill_columns(row, div_active_b, Column::CpuDivActiveH);
            trace.fill_columns_base_field(row, &[BaseField::from(gate_div_v)], Column::GateDivH);
            trace.fill_columns_base_field(row, &[BaseField::from(gate_rem_v)], Column::GateRemH);
            let active_v: u32 = if div_active_b { 1 } else { 0 };
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    (active_v * gate_div_v).rem_euclid(stwo::core::fields::m31::P),
                )],
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    (active_v * gate_rem_v).rem_euclid(stwo::core::fields::m31::P),
                )],
                Column::DivActiveRemH,
            );
            trace.fill_columns(
                row,
                flags.is_div_rem && flags.is_32bit,
            ...
            trace.fill_columns(
                row,
                flags.is_div_rem && vd_zero_b,
                Column::IsDivRemTimesVdzH,
            );
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    (dbz_active_v * gate_div_v).rem_euclid(stwo::core::fields::m31::P),
                )],
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    (dbz_active_v * gate_rem_v).rem_euclid(stwo::core::fields::m31::P),
                )],
            ...
            trace.fill_columns_base_field(row, &pnz_msb_times_ind, Column::PartNZMsbTimesIndH);
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    tz_lo4.rem_euclid(stwo::core::fields::m31::P),
                )],
                Column::TzLo4H,
            );
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    tz_hi4.rem_euclid(stwo::core::fields::m31::P),
                )],
                Column::TzHi4H,
            );
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    lz_64.rem_euclid(stwo::core::fields::m31::P),
                )],
                Column::Lz64H,
            );
            ...
            trace.fill_columns_base_field(
                row,
                &[BaseField::from(
                    lz_32.rem_euclid(stwo::core::fields::m31::P),
                )],
                Column::Lz32H,
            );
        ...
        let canonical_flags = super::classify::classify_opcode_for_program_memory(step.opcode);
        ...
        trace.fill_columns(
            row,
            val_b_is_reg_b && !is_truncated_b,
            Column::ValBIsRegNotTruncH,
        );
        trace.fill_columns(
            row,
            val_b_is_reg_b && is_truncated_b,
            Column::ValBIsRegTruncH,
        );
        trace.fill_columns(row, val_d_is_reg_b, Column::ValDIsRegH);
        trace.fill_columns(row, val_d_is_reg_b && !is_shift_c_b, Column::NonShiftGateH);
        trace.fill_columns(
            row,
            val_d_is_reg_b && !is_shift_c_b && !is_truncated_b,
        ...
        trace.fill_columns(
            row,
            val_d_is_reg_b && is_shift_c_b,
            Column::ValDIsRegShiftCH,
        );
        ...
        let prod =
            (phi7_field_u32 as u64 * phi7_inv_field_u32 as u64) % stwo::core::fields::m31::P as u64;
        use stwo::core::fields::m31::BaseField;
        ...
        trace.fill_columns_base_field(row, &[BaseField::from(pop_hi)], Column::Is64bitPopcountHiH);
