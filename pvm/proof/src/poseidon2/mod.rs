...
//! one orphan-legal marker a custom channel needs on a backend; the same trick
//! satisfies `BackendForChannel<P2MerkleChannel>` on `SimdBackend` too (see
//! [`simd_backend`]), so the recursion stack proves on `SimdBackend` — only the
//! scalar merkle hash drops to `CpuBackend`, the rest of the prove stays SIMD.
//!
...
/// Fiat-Shamir). Contrast the commitment-only `P2MerkleChannel`, whose `C` is
/// still `Blake2sM31Channel` (it only swapped the commitment hash).
#[derive(Default)]
...
// ── SimdBackend `BackendForChannel<P2MerkleChannel>` (the prove-TIME lever) ──
//
// Production proves on `SimdBackend` (vectorized FRI / quotients / twiddles).
// Satisfying `BackendForChannel<P2MerkleChannel>` on `SimdBackend` lets the
// recursion (Poseidon2-M31) stack prove on `SimdBackend` too — so the whole
// prove vectorizes and only the merkle commitment stays scalar. Retargeting
// everything to `CpuBackend` instead forfeits SIMD on the entire prove (FRI,
// quotients, twiddles) and proves an order of magnitude slower.
//
// All four impls are orphan-legal for the same reason as the `CpuBackend` one
// above: each names a local type parameter (`P2Hash` / `P2MerkleHasher` /
// `Poseidon2M31Channel` / `P2MerkleChannel`). The `MerkleOpsLifted` /
// `GrindOps` bodies delegate to `CpuBackend` (the columns are moved to CPU at
// the merkle boundary, exactly as the old commit-transplant did, but only for
// the hash — the rest of the prove stays SIMD); roots are therefore
// bit-identical to the `CpuBackend` path, so existing proofs still verify.
#[cfg(feature = "prover")]
mod simd_backend {
    use alloc::vec::Vec;
    ...
    use stwo::prover::backend::{BackendForChannel, Col, Column, ColumnOps, CpuBackend};
    use stwo::prover::vcs_lifted::ops::MerkleOpsLifted;

    use super::{P2Hash, P2MerkleChannel, P2MerkleHasher, Poseidon2M31Channel};

    // Hash columns are stored as a plain `Vec` (no SIMD packing applies to a
    // digest type), matching how `SimdBackend` stores its `Poseidon252` /
    // `Keccak256` hash columns. Never bit-reversed (hashes only live in the
    // commitment tree, not the evaluation domain).
    impl ColumnOps<P2Hash> for SimdBackend {
        type Column = Vec<P2Hash>;
        fn bit_reverse_column(_column: &mut Self::Column) {
            unimplemented!("P2Hash columns are never bit-reversed")
        }
    }
    ...
    impl MerkleOpsLifted<P2MerkleHasher> for SimdBackend {
        fn build_leaves(
            ...
            // — group columns by domain size, lift smaller layers, absorb in
            // rate-sized chunks, finalize — but each leaf's hasher is
            // independent, so the per-row loops run on rayon instead of
            // serially. Leaf hashing is the prove-TIME bottleneck (the
            // Poseidon2-M31 commit dominates a light prove), and stwo's
            // `CpuBackend` leaf loop is serial; parallelizing it is the actual
            // speedup. Roots are bit-identical to the serial path.
            let cpu_cols: Vec<Vec<BaseField>> = columns.iter().map(|c| c.to_cpu()).collect();
            build_leaves_row_parallel(&cpu_cols, lifting_log_size)
        }

        fn build_next_layer(prev_layer: &Col<Self, P2Hash>) -> Col<Self, P2Hash> {
            // `CpuBackend`'s `build_next_layer` is already rayon-parallel.
            <CpuBackend as MerkleOpsLifted<P2MerkleHasher>>::build_next_layer(prev_layer)
        }
    }

    /// Row-parallel reimplementation of stwo's `CpuBackend` lifted-leaf hashing
    /// for the Poseidon2-M31 hasher. Columns are assumed in increasing domain
    /// size (the commitment scheme's contract). Each output leaf runs its own
    ...
    fn build_leaves_row_parallel(columns: &[Vec<BaseField>], lifting_log_size: u32) -> Vec<P2Hash> {
        ...
        // One hasher per leaf in the current (smallest) layer.
        let mut prev_layer: Vec<P2MerkleHasher> = alloc::vec![P2MerkleHasher::default(); 2];
        let mut prev_layer_log_size: u32 = 1;

        // Walk consecutive runs of equal-`ilog2`-length columns (the manual
        // form of the `CpuBackend` `group_by`, avoiding an itertools dep).
        let mut start = 0;
        while start < columns.len() {
            let log_size = columns[start].len().ilog2();
            let mut end = start + 1;
            while end < columns.len() && columns[end].len().ilog2() == log_size {
                end += 1;
            }
            let group = &columns[start..end];

            // Lift the previous (smaller) layer's hashers up to this group's
            // domain size, cloning the partial sponge state per the standard
            // bit-reversed lifting index.
            ...
            // Absorb this group's columns in rate-sized (16-column) chunks,
            // one independent hasher per leaf row.
            for chunk in group.chunks(16) {
                prev_layer
                    .par_iter_mut()
                    .enumerate()
                    .for_each(|(i, hasher)| {
                        let row: Vec<BaseField> = chunk.iter().map(|col| col[i]).collect();
        ...
        // Final lift to `lifting_log_size`, then finalize every leaf.
        let log_ratio = lifting_log_size.saturating_sub(prev_layer_log_size);
        if lifting_log_size > prev_layer_log_size {
        ...
        prev_layer
            .into_par_iter()
            .map(MerkleHasherLifted::finalize)
            .collect()
    }

    // The proof-of-work grind reuses `CpuBackend`'s generic
    // `verify_pow_nonce`-loop grind (no SIMD Poseidon2-M31 permutation yet).
    ...
    impl BackendForChannel<P2MerkleChannel> for SimdBackend {}
}
