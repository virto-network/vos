...
//!
//! The trace shape uses the shared `vos_pvm_proof::bench_helpers::add_side_note` helper
...
use vos_pvm_proof::bench_helpers::add_side_note;
...
use vos_pvm_proof_verifier::{verify_standalone_with_pcs_policy, CommitmentHash, Proof};
...
fn binding_fixture(dir: &std::path::Path, io_hash_hex: &str) {
    ...
    if std::env::var("VWB_DUMP_STEPS").is_ok() {
        ...
        for (row, e) in vos_pvm_proof::chips::register_memory::build_entries_from_side_note(&side_note)
            .iter()
            .enumerate()
        {
            eprintln!(
