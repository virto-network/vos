...
fn prove_unconditional_jump() {
    ...
    // In vos_pvm, basic block starts are computed from terminators. Jump at offset 0 is a terminator,
...
fn load_imm_jump_ind_positive_smoke() {
    ...
    let mut side_note =
        vos_pvm_proof::SideNote::new(steps, code, bitmask).with_jump_table(vec![3u32, 5u32]);
    let proof = prove(&mut side_note).expect("proving failed");
    verify(proof, &side_note).expect("verification failed");
}
