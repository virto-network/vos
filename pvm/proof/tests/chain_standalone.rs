...
//! (the trustless counterpart to `vos_pvm_proof::verify_chain`).
//!
...
use vos_pvm_proof::{SideNote, program_commitment_of_proof, prove};
use vos_pvm_proof_verifier::verify_chain_standalone;

/// Six chained Add64s + Trap (7 steps), cut into [0..3) and [3..7).
fn prove_two_segment_chain() -> (vos_pvm_proof::Proof, vos_pvm_proof::Proof) {
    let code = vec![
        Opcode::Add64 as u8,
        0x10,
        2,
...
fn chain_standalone_accepts_honest_chain_and_rejects_forgeries() {
    ...
    // The chain anchors on proofs[0]'s entering RAM root (the memory
