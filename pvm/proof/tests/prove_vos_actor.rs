...
fn load_fibonacci_blob() -> Option<Vec<u8>> {
    ...
    Some(vos_pvm_compiler::link_elf(&elf_data).expect("failed to transpile fibonacci ELF"))
}

/// Test-side panicking wrapper over `vos_pvm_proof::actor::interpreter_from_blob`
...
fn interpreter_from_blob(blob: &[u8], gas: u64) -> (Interpreter, Vec<u8>) {
    vos_pvm_proof::actor::interpreter_from_blob(blob, gas).expect("interpreter from blob")
}
...
fn load_actor_blob(name: &str) -> Option<Vec<u8>> {
    ...
    let elf_path = format!("{base}{name}/target/riscv64em-vos/release/{name}.elf");
    let elf_data = match std::fs::read(&elf_path) {
        Ok(b) => b,
        Err(_) => {
    ...
    Some(vos_pvm_compiler::link_elf(&elf_data).expect("failed to transpile ELF"))
}
...
fn profile_hash_variant(name: &str) {
    ...
    let blob = match std::fs::read(&pvm_path) {
        ...
        Err(_) => match std::fs::read(&elf_path) {
            Ok(elf) => vos_pvm_compiler::link_elf(&elf).expect("transpile"),
            Err(_) => {
...
fn debug_blake2s_prefix() {
    ...
    let config = vos_pvm_proof::PcsConfig {
        pow_bits: 5,
        fri_config: vos_pvm_proof::FriConfig::new(0, 1, 3, 1),
...
fn prove_blake2b_precompile() {
    use vos_pvm_proof::chips::blake2b::blake2b_compress;
    use vos_pvm_proof::core::tracing::ECALL_BLAKE2B_COMPRESS;
    ...
    let code = vec![
        vos_pvm::instruction::Opcode::Ecalli as u8,
        ECALL_BLAKE2B_COMPRESS as u8,
        0,
        0,
        0,
        vos_pvm::instruction::Opcode::Trap as u8,
    ];
    ...
    let pvm = vos_pvm::interpreter::Interpreter::new(
        code.clone(),
        bitmask.clone(),
        vec![],
        regs,
    ...
    let proof =
        vos_pvm_proof::prove_with_config(&mut side_note, config).expect("blake2b proving failed");
    let policy = vos_pvm_proof::PcsPolicy {
        min_pow_bits: 5,
        min_fri_queries: 3,
        min_fri_log_blowup: 0,
    };
    vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy)
...
fn prove_vos_halt_jump_without_a_program_jump_table() {
    ...
    let mut side_note =
        vos_pvm_proof::actor::trace_blob(&blob, 1_000_000).expect("trace the canonical halt jump");
    ...
    let proof = vos_pvm_proof::prove_with_config(&mut side_note, config)
...
fn prove_verify_ristretto_isolated(side_note: &mut vos_pvm_proof::SideNote) -> bool {
    let config = vos_pvm_proof::PcsConfig {
        pow_bits: 5,
        fri_config: vos_pvm_proof::FriConfig::new(0, 1, 3, 1),
        lifting_log_size: None,
    };
    let components: &[&'static dyn vos_pvm_proof::harness::MachineProverComponent] = &[
        &vos_pvm_proof::chips::RangeMultiplicity256,
        &vos_pvm_proof::chips::RistrettoChip,
    ];
    let Ok(proof) = vos_pvm_proof::prove_with_explicit_components(side_note, config, components)
    else {
        return false;
    };
    let policy = vos_pvm_proof::PcsPolicy {
    ...
    let verifier_components: Vec<&dyn vos_pvm_proof::harness::MachineComponent> = components
        ...
        .collect();
    vos_pvm_proof::verify_with_explicit_components(
        proof,
        side_note,
        &verifier_components,
        components,
...
/// rejected.
fn prove_field_op_row_isolated(row: vos_pvm_proof::chips::ristretto::witness::FieldOpRow) -> bool {
    use vos_pvm_proof::chips::ristretto::witness::{fill_input, fill_output};

    let mut side_note = vos_pvm_proof::SideNote::new(Vec::new(), Vec::new(), Vec::new());
...
fn prove_ristretto_chip_field_add() {
    use vos_pvm_proof::chips::ristretto::witness::fill_add;
...
fn prove_ristretto_chip_field_mul() {
    use vos_pvm_proof::chips::ristretto::witness::fill_mul;
...
fn ristretto_chip_negative_per_row_soundness_audit() {
    use vos_pvm_proof::chips::ristretto::witness::{FieldOpRow, fill_add, fill_mul, fill_sub};
    ...
    let mut bad = fill_add(a_small, b_small);
    bad.out = vos_pvm_proof::chips::ristretto::field::P_BYTES;
    // Recompute final-form: p - p - 1 = -1 < 0, so ff_brw[0] = 1 immediately.
    let mut bw: i16 = 1;
    for i in 0..32 {
        let p_i = vos_pvm_proof::chips::ristretto::field::P_BYTES[i] as i16;
...
fn ristretto_chip_per_payment_row_sequence_composes() {
    use vos_pvm_proof::chips::ristretto::point::{
        ED25519_TWO_D, ExtendedPoint, point_add_rows, point_identity, scalar_mult_rows,
    };
    use vos_pvm_proof::chips::ristretto::witness::{FieldOpRow, validate_row_sequence_composes};
...
fn ristretto_chip_unrelated_rows_now_rejected() {
    ...
    let prove_result = vos_pvm_proof::prove_with_config(&mut side_note, config);
    let policy = vos_pvm_proof::PcsPolicy {
        min_pow_bits: 5,
        min_fri_queries: 3,
        min_fri_log_blowup: 0,
    ...
    let rejected = match prove_result {
        Err(_) => true,
        Ok(proof) => vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy).is_err(),
    };
    assert!(
        rejected,
...
fn prove_ristretto_chip_with_input_producers() {
    use vos_pvm_proof::chips::ristretto::witness::{fill_add, fill_input};
    ...
    let chain_balanced = match prove_result {
        Err(_) => false,
        Ok(proof) => vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy).is_ok(),
    };
...
fn prove_ristretto_chip_closed_chain_input_output() {
    use vos_pvm_proof::chips::ristretto::witness::{fill_add, fill_input, fill_output};
    ...
    let proof = vos_pvm_proof::prove_with_explicit_components(&mut side_note, config, components)
...
fn prove_blake2b_via_ecall() {
    ...
    let mut side_note =
        vos_pvm_proof::SideNote::new(steps, code.clone(), bitmask.clone()).with_memory(flat_mem);
    ...
    let proof = vos_pvm_proof::prove_with_config(&mut side_note, config).expect("proving failed");
    ...
    vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy).expect("verification failed");
    eprintln!(
...
fn prove_ristretto_via_ecall_boundary() {
    ...
    let mut side_note = vos_pvm_proof::SideNote::new(steps, code.clone(), bitmask.clone())
...
fn prove_ristretto_point_add_via_ecall_boundary() {
    ...
    let proof =
        vos_pvm_proof::prove_with_config(&mut side_note, config).expect("point_add proving failed");
    let policy = vos_pvm_proof::PcsPolicy {
        min_pow_bits: 5,
        min_fri_queries: 3,
        min_fri_log_blowup: 0,
    };
    vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy).expect("verify");
}
...
fn prove_two_ristretto_scalar_mult_ecalls() {
    ...
    let proof =
        vos_pvm_proof::prove_with_config(&mut side_note, config).expect("two-ecall proving failed");
    let policy = vos_pvm_proof::PcsPolicy {
        min_pow_bits: 5,
        min_fri_queries: 3,
        min_fri_log_blowup: 0,
...
fn prove_ristretto_chip_scalar_mult_chained_small() {
    ...
    use vos_pvm_proof::chips::ristretto::field;
