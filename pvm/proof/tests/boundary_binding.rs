...
use vos_pvm_proof::{SegmentState, SideNote, program_commitment_of_proof, prove};
...
fn from_scratch_boundary_forgery_is_rejected() {
    ...
    vos_pvm_proof::verify(honest.clone(), &side_note).expect("honest proof must verify");
    ...
    let variants: &[(&str, Forge)] = &[
        ...
        // The entering / exit memory-page Merkle roots are bound by
    ...
    for (name, forge) in variants {
        ...
        let forged =
            vos_pvm_proof::prove_with_boundary_override(&mut sn, forged_initial, forged_final)
                .expect("the from-scratch prover completes its (lying) prove");

        assert!(
            vos_pvm_proof::verify(forged.clone(), &sn).is_err(),
...
fn boundary_override_with_honest_values_verifies() {
    ...
    let overridden = vos_pvm_proof::prove_with_boundary_override(
        &mut sn,
        honest.initial_state.clone(),
        honest.final_state.clone(),
    )
    .expect("override prove failed");
    vos_pvm_proof::verify(overridden.clone(), &sn).expect("honest-values override must verify");
