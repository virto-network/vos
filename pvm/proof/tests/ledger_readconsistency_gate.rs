...
//! read-after-write via `is_read · (value − prev_value) = 0`.  That constraint
//! alone is insufficient: were `prev_value` a FREE witness column — nothing
//! (`#[mask_next_row]`) tying it to the previous ledger row's value, and no
//! `(key, ts)` sortedness check — a from-scratch prover could fill a read row
//! with a lie `L` and set the same row's `prev_value := L`, so read-consistency
//! holds (`L − L = 0`) while the *actual* previous ledger row carries a
//! different value `T`.  That would forge any register/memory read, including
//! the closing read that pins `final_state.registers` and hence the voucher
//! io-hash.
//!
//! These tests build an HONEST component trace, then tamper a single read
//! row's `Value` + `PrevValue` cells (bypassing the honest trace filler) and
//! assert the chip's AIR constraints REJECT it.  The cross-row `prev_value`
//! binding + `(key, ts)` sortedness are what make the AIR reject that forgery
//! directly.  Unlike `register_ledger_negative.rs` /
...
use vos_pvm_proof::chips::{MemoryChip, RegisterMemoryChip};
...
use vos_pvm_proof::harness::MachineProverComponent;
...
// Asserts a forged read is rejected: the cross-row prev_value binding +
// (reg,ts) sortedness + is_write tuple-binding tie the read to the prior
// ledger row's value.  See docs/plans/ledger-read-consistency.md.
#[test]
fn register_forged_read_value_is_rejected() {
    use vos_pvm_proof::chips::register_memory::Column;
...
fn memory_side_note() -> SideNote {
    ...
    let mut sn = SideNote::new(steps, code, bitmask);
    // MemoryChip injects the per-page `ts=0` boundary writes + closing reads
    // and enforces the group-start/group-end constraints, so the honest trace
    // requires the memory-page payload the prove path builds.
    sn.ingest_memory_pages();
    sn
}

// Asserts a forged read is rejected: the cross-row prev_value binding +
// (addr,ts) sortedness tie the read to the prior ledger row's value.  See
// docs/plans/ledger-read-consistency.md.
#[test]
fn memory_forged_read_value_is_rejected() {
    use vos_pvm_proof::chips::memory::Column;
