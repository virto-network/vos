...
//! Chip-isolated prove/verify harness smoke tests.
//!
...
//! broken and no chip-isolated validation can proceed.
...
/// - Stwo accepts a bound-1 AIR cleanly.
///
/// What this does NOT validate:
/// - High-bound chip flattening (Blake2b/Mul/DivRem/Cpu/Ristretto are
///   covered by their own isolated harnesses).
/// - Lookup balance closure across chips (that needs sink components —
...
fn harness_smoke_bound1_only() {
    ...
    let verifier_components: Vec<&dyn vos_pvm_proof::harness::MachineComponent> =
        BOUND1_HARNESS_COMPONENTS
            .iter()
            .map(|c| *c as &dyn vos_pvm_proof::harness::MachineComponent)
            .collect();

    // Cheap policy matching the cheap PcsConfig — production verify
    // would use PcsPolicy::STANDARD; the harness accepts its own cheap
    // config to keep chip-isolated validation cycles fast.
...
/// Blake2bChip-isolated harness.  Asserts:
///
/// - `prove_with_explicit_components([&Blake2bChip], ...)` SUCCEEDS
///   (the chip's algebraic constraints are all degree ≤ 2).
...
///   validation (lookup tuple correctness, which the full-system
///   integration tests already cover).
/// - The full-system path (`prove_add64`) checks lookup correctness
///   end-to-end.
///
/// An upstream Stwo bug — `MerkleProverLifted::decommit` index OOB on
/// mixed-width column traces — is documented in
/// `STWO_MERKLE_LIFTED_OOB_ISSUE_DRAFT.md` (not blocking: bound-1 chips
/// and this harness shape don't trip it, and production paths never
/// did).
#[test]
fn harness_blake2b_isolated() {
    use vos_pvm_proof::chips::Blake2bCall;
    ...
    let verifier_components: Vec<&dyn vos_pvm_proof::harness::MachineComponent> = components
        .iter()
        .map(|c| *c as &dyn vos_pvm_proof::harness::MachineComponent)
        .collect();
...
/// `MemoryPageChip` / `MemoryMerkleChip` are not included here).
#[test]
...
/// PVM step.  Validates that CpuChip's algebra is sound.  Verify is
/// open-chain — lookups don't close without producer chips, but prove
/// reaching SUCCESS means the OODS sanity check passes (no constraint
/// algebra regressions).
///
/// Note on GateDivH padding-row fill: the helper
/// `(DivRemOp - 2)·(DivRemOp - 3)` is unconditionally constrained, so
/// padding rows where DivRemOp=0 require GateDivH = 6, not 0.
#[test]
...
/// RistrettoChip-isolated harness.  Prove `[&RistrettoChip]` alone with
/// one `ECALL_RISTRETTO_SCALAR_MULT` step (the canonical 2·G test).
///
/// Asserts:
...
///
/// What this validates:
/// - The `ScalarMultKind`/ECALL plumbing does not regress the
///   variable-base chip-isolated proving path.
/// - A regression net for the variable-base ladder, distinct from the
///   fixed-base comb-method path covered by the sibling comb harnesses.
#[test]
fn harness_ristretto_isolated() {
    ...
    // drives RistrettoChip's variable-base ladder.  `FixedBasepoint`
    // records route onto the comb path, bypassing RistrettoChip — using
    // a fixed-base record here would leave ristretto_field_rows empty
    // and trivialize this harness.
    ...
    // grey-transpiler's `map_register` and the host handler's φ[7/8/9]
    // reads in `tracing.rs`.
    regs[7] = scalar_addr;
    regs[8] = point_addr;
    regs[9] = output_addr;
    ...
    // FixedBasepoint records route onto the comb path, so this harness
    // uses Variable to keep exercising RistrettoChip.
    use vos_pvm_proof::core::tracing::ScalarMultKind;
...
/// RistrettoCombTableChip-isolated harness.  Validates the precomputed
/// comb-table preprocessed columns (1024 rows of
/// `T[i][j] = j · 2^(4·i) · G`) and the table-side lookup constraint
/// emit cleanly under prove + verify.
///
/// All multiplicities default to zero — no consumer chip in scope.
/// Lookup contribution is `-0 = 0` per row, so the relation balance is
/// trivially zero and verify accepts.
///
/// What this validates:
...
/// - The chip's algebraic constraints (only the relation, no others)
///   close cleanly.
#[test]
...
/// Comb-method lookup-balance harness.
///
...
///
/// `RistrettoCombScalarBoundaryChip` also emits a +IsReal
/// `MemoryAccessLookupElements` producer per row to pin the scalar
/// bytes to PVM memory.  Since this harness does not include
/// `MemoryChip` / `MemoryBoundaryChip` in scope, that producer goes
/// unbalanced and verify rejects open-chain.  The fully-closed chain is
/// exercised in `harness_ristretto_fixed_base_e2e_with_memory` below;
/// this harness serves as a regression net for the
...
/// REAL PVM trace through the full active component set.  With the
/// ristretto-`ts` binding, RistrettoEcallChip consumes a `RistrettoCall`
/// (RELATION A) tuple per call that only CpuChip produces, so the comb-chain +
/// ledger balance cannot be exercised with a synthetic side_note — a real
/// CpuChip trace is required.  This validates: the scalar-boundary / output
/// producers pair cleanly with MemoryChip; the skip-scalar-bytes split keeps
/// the ledger balanced; and the RELATION-A + Tier-2 ts anchoring closes
/// end-to-end.
#[test]
...
/// Multi-call coverage for the comb chip pair.
///
...
/// Soundness regression net for the scalar / PVM-memory binding.
/// Synthesizes a call whose
...
fn harness_ristretto_scalar_memory_mismatch_rejected() {
    ...
    use vos_pvm_proof::core::tracing::{RistrettoMemOp, ScalarMultKind};
    use vos_pvm_proof::side_note::RistrettoCombCall;
    ...
    let components: &[&'static dyn MachineProverComponent] = &[
        ...
        &vos_pvm_proof::chips::RangeMultiplicity256,
        &vos_pvm_proof::chips::BitwiseLookupChip,
        &RistrettoEcallChip,
        &RistrettoCombTableChip,
        &RistrettoCombAnchorChip,
        &RistrettoCombScalarBoundaryChip,
...
fn harness_ristretto_consumer_isolated_empty() {
    use vos_pvm_proof::chips::RistrettoFixedBaseConsumerChip;
...
fn harness_ristretto_compress_isolated_empty() {
    use vos_pvm_proof::chips::RistrettoCombCompressChip;
...
/// Soundness regression for the OUTPUT BINDING.
...
/// The Ristretto IDENTITY (`0·G`) must prove + verify through the full
/// comb→compress→output chain.
///
/// This is the case that gates conservation-of-value: every balanced
...
/// `[0;32]` (the canonical identity encoding).  Without the `IsIdentity`
/// gate this proof would fail with `ConstraintsNotSatisfied` on the
/// unity row.
#[test]
...
/// The identity encoding is BOUND: a prover cannot pass off a
/// non-`[0;32]` output for the identity point.  The compress
