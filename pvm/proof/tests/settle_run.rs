...
fn settle_verify_accepts_on_pvm() {
    let elf_path = concat!(
        ...
        "/settlement-verifier/target/riscv64em-vos/release/settle.elf"
    );
    ...
    let blob = vos_pvm_compiler::link_elf(&elf).expect("transpile settle.elf");

    // Generous gas; the verify is the whole point, so we don't want to clip it.
    let gas: u64 = 100_000_000_000;
    let (interp, _mem) = vos_pvm_proof::actor::interpreter_from_blob(&blob, gas)
        .expect("build interpreter from blob");
