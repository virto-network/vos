...
mod imp {
    ...
    /// Panicking wrapper over `vos_pvm_proof::actor::interpreter_from_blob` — every
    ...
    fn profile_actor_with_config(name: &str, gas: u64, config: vos_pvm_proof::PcsConfig) {
        let Some(blob) = load_actor_blob(name) else {
            return;
        };
        let parsed = program::parse_blob(&blob).expect("parse blob");
        ...
        let t = std::time::Instant::now();
        vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &vos_pvm_proof::PcsPolicy::MOBILE)
    ...
    fn prove_segmented_hash_bench() {
        ...
        vos_pvm_proof::verify_chain(&[proof1, proof2], &[&seg1_sn, &seg2_sn])
    ...
    fn bench_ristretto_chip_soundness_complete_chain() {
        ...
        let t = Instant::now();
        let proof =
            vos_pvm_proof::prove_with_explicit_components(&mut side_note, config, components)
    ...
    fn bench_ristretto_chip_combined_with_cpu_baseline() {
        ...
        use vos_pvm_proof::chips::ristretto::point::{
            point_add_rows, point_identity, scalar_mult_rows,
        };
        use vos_pvm_proof::core::tracing::TracingPvm;
    ...
    fn bench_ristretto_chip_one_private_payment() {
        ...
        let t3 = Instant::now();
        vos_pvm_proof::verify_with_pcs_policy(proof, &side_note, &policy).expect("verify failed");
