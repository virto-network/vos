...
impl SpaceAuthority {
    ...
    fn mutate_role(&mut self, mutation: Vec<u8>, signature: Vec<u8>) -> bool {
        ...
        let Some((holder_kind, holder_bytes)) = holder_key(holder) else {
            return false;
        };
        ...
        let index = self
            .grants
            .iter()
            .position(|row| row.holder_kind == holder_kind && row.holder == holder_bytes);
    ...
    fn authorize_role(&self, claim: Vec<u8>) -> Vec<u8> {
        ...
        let Some((holder_kind, holder)) = holder_key(claim.holder) else {
            return Vec::new();
        };
        let Some(row) = self
            .grants
            .iter()
            .find(|row| row.holder_kind == holder_kind && row.holder == holder)
        else {
            return Vec::new();
        };
