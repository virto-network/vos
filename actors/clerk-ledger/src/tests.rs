...
#[test]
fn smt_root_matches_mem_ledger_for_same_content() {
    // Byte-equality pin against the reference impl. If
    // cipher-clerk's leaf encoding or composite root shape
    // shifts, this test fires and we know to update either the
    // local helpers or pin a different cipher-clerk rev.
    use cipher_clerk::helpers::{MemLedger, MemOracle};

    let registrar = AuthKey([0xAA; 32]);
    let code: u16 = 7;

    // ── clerk-ledger side ────────────────────────────────────
    let mut accounts = Vec::new();
    let mut transfers = Vec::new();
    // Build a journal with a FIXED id (not random) so it
    // matches the MemLedger journal id below.
    let journal = CcJournal::new(CcJournalId([0xCA; 16]), registrar, code);
    // Three accounts.
    for byte in [0x01, 0x02, 0x03] {
        let mut a = CcAccount::new(
            CcAccountId([byte; 16]),
            journal.id,
            AuthKey([byte; 32]),
            1u32,
        ...
        // Match what apply_account_creations would stamp.
        a.timestamp = 1_000;
        accounts.push(a);
    }
    // One transfer pointing at journal so the cross-ref leaves
    // are exercised. (No entries — apply_batch would normally
    // fill these; for root-equality we just need bytes that
    // round-trip identically through both archives.)
    let mut t = CcTransfer::default();
    t.id = CcTransferId([0x10; 16]);
    t.journal_id = journal.id;
    t.correlation_id = t.id;
    transfers.push(t.clone());
    let our_root = compute_state_root(&accounts, &transfers, Some(&journal));

    // ── reference side ──────────────────────────────────────
    let mut mem = MemLedger::new();
    let mut oracle = MemOracle::new();
    // Force MemLedger's journal to have the same id as ours so
    // the journals_smt leaves match byte-for-byte. The default
    // bootstrap_journal randomises; we want determinism here.
    mem.journals_smt.insert(journal.id.0, &{
        let archive = vos::rkyv::to_bytes::<vos::rkyv::rancor::Error>(&journal)
            .expect("journal rkyv encode");
        let mut out = Vec::with_capacity(archive.len() + 1);
        out.push(2); // LEAF_JOURNAL — same domain tag as smt.rs
        out.extend_from_slice(archive.as_ref());
        out
    });
    mem.journals.insert(journal.id.0, journal.clone());
    for a in &accounts {
        mem.put_account(a.clone(), &mut oracle);
    }
    mem.put_transfer(t.clone(), &mut oracle);
    let reference_root = mem.root();

    assert_eq!(
        our_root, reference_root,
        "clerk-ledger SMT root must match cipher_clerk::helpers::MemLedger byte-for-byte"
    );
}
