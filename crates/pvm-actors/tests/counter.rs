#![allow(dead_code)]
use pvm_actors::{Actor, ActorId, Context, Executor, Mailbox, Progress};
...
impl pvm_actors::Message<Increment> for Counter {
    type Reply = i32;
...
impl pvm_actors::Message<GetCount> for Counter {
    type Reply = i32;
...
impl pvm_actors::Message<Reset> for Counter {
    type Reply = ();
    ...
    async fn deliver(self, actor: &mut Counter, ctx: &mut Context<Counter>) {
        match self {
            CounterMsg::Increment(msg) => {
                let _ = <Counter as pvm_actors::Message<Increment>>::handle(actor, msg, ctx).await;
            }
            CounterMsg::GetCount(msg) => {
                let _ = <Counter as pvm_actors::Message<GetCount>>::handle(actor, msg, ctx).await;
            }
            CounterMsg::Reset(msg) => {
                let _ = <Counter as pvm_actors::Message<Reset>>::handle(actor, msg, ctx).await;
            }
        }
    }
}
