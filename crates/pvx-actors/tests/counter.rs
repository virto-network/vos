#![allow(dead_code)]
use pvx_actors::{Actor, ActorId, Context, Executor, Mailbox, Progress};
...
impl pvx_actors::Message<Increment> for Counter {
    type Reply = i32;
...
impl pvx_actors::Message<GetCount> for Counter {
    type Reply = i32;
...
impl pvx_actors::Message<Reset> for Counter {
    type Reply = ();
    ...
    async fn deliver(self, actor: &mut Counter, ctx: &mut Context<Counter>) {
        match self {
            CounterMsg::Increment(msg) => {
                let _ = <Counter as pvx_actors::Message<Increment>>::handle(actor, msg, ctx).await;
            }
            CounterMsg::GetCount(msg) => {
                let _ = <Counter as pvx_actors::Message<GetCount>>::handle(actor, msg, ctx).await;
            }
            CounterMsg::Reset(msg) => {
                let _ = <Counter as pvx_actors::Message<Reset>>::handle(actor, msg, ctx).await;
            }
        }
    }
}
...
fn block_on<F: core::future::Future>(mut fut: F) -> F::Output {
    ...
    loop {
        match fut.as_mut().poll(&mut cx) {
            ...
            Poll::Pending => {} // In real PVM, host would schedule other work
        }
    }
}
...
fn single_actor_processes_messages() {
    block_on(async {
        ...
        // Poll — processes one message per actor per call
...
fn multiple_actors_round_robin() {
    block_on(async {
        ...
        // One tick processes both actors
        assert_eq!(poll_counters(&mut exec).await, Progress::Progressed);
