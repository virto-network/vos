...
// ── Actor ──────────────────────────────────────────────────────────
//
// `#[messages]` lives here in the lib so consumers can `use
// registry::RegistryRef` and drive the registry with typed
// methods — same Ref works from PVM actor handlers (with `ctx`
// as the invoker) and from host code (with `&node`, gated on
// vos's `std` feature). The PVM entry emission lives in
// `main.rs` so cross-actor `use registry::*` doesn't pull
// duplicate `_start` symbols into the dependent's link.
...
impl Registry {
    ...
    /// rkyv-encoded bytes; the generated `RegistryRef::lookup`
    ...
    /// `RegistryRef::list -> Page` on the host side.
    #[msg]
...
// the hyperspace name. The typed `RegistryRef` is generated by
// `#[messages]` above and is available unconditionally; only
// this host-side helper requires the `host` feature.
//
...
// `RegistryRef` (the typed reference) is generated by
// `#[messages]` above — one async method per `#[msg]`, all
// returning `Result<HandlerReturnType, vos::actors::client::ClientError>`.
