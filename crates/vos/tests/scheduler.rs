...
/// A simple in-process driver for testing.
struct TestDriver {
    states: [i32; 4],
    initialized: [bool; 4],
}
...
impl Driver<TestMsg> for TestDriver {
    ...
    fn handle(&mut self, id: ServiceId, msg: &TestMsg) -> Status {
        let idx = Self::idx(id);
        match msg {
    ...
    fn poll(&mut self, _id: ServiceId) -> Status {
        Status::Ready
    }

    fn drop_service(&mut self, _id: ServiceId) {}
}
