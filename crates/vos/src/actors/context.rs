...
/// A restricted [`Context`] for actors declared with `#[actor(pure)]`
/// and `#[messages(pure)]`.
///
/// `PureContext` exposes only the deterministic subset of `Context`:
/// state mutations (`store`, `spawn`, `provide`, `install`), one-way
/// messaging (`tell`, `send`, `send_self`, `tell_raw`), cooperative
/// scheduling (`yield_now`, `sleep`), and lifecycle introspection.
///
/// **Missing by design** — methods that observe external state and
/// would break deterministic replay under a CRDT/Raft strategy:
///
/// - [`Context::ask`], [`Context::ask_raw`]
/// - [`Context::resolve`], [`Context::tell_named`]
/// - [`Context::fetch`], [`Context::host_call`]
///
/// A handler on a pure actor that calls any of the above fails to
/// compile — the trustless mini-chain guarantee is enforced by the
/// type system, not by runtime assertions.
#[repr(transparent)]
pub struct PureContext<A: Actor>(Context<A>);

impl<A: Actor> PureContext<A> {
    /// Borrow a `Context` as a `PureContext`. The macro-generated
    /// handler wrappers use this to restrict the type seen inside a
    /// pure actor's handler body.
    #[doc(hidden)]
    pub fn wrap(ctx: &mut Context<A>) -> &mut PureContext<A> {
        // SAFETY: `#[repr(transparent)]` guarantees identical layout
        // between `Context<A>` and `PureContext<A>`. The reborrow
        // inherits the lifetime of `ctx`, so no aliasing is introduced.
        unsafe { &mut *(ctx as *mut Context<A> as *mut PureContext<A>) }
    }
    ...
    /// Send raw bytes to another service (fire-and-forget).
    pub fn tell_raw(&mut self, target: ServiceId, payload: &[u8]) {
        self.0.tell_raw(target, payload);
    }

    /// Send a typed message to another service.
    pub fn send<M: super::codec::Encode>(&mut self, target: ServiceId, msg: &M) {
        self.0.send(target, msg);
    }

    /// Send a typed message to self.
    pub fn send_self<M: super::codec::Encode>(&mut self, msg: &M) {
        self.0.send_self(msg);
    }

    /// Send a dynamic message to another actor (fire-and-forget).
    pub fn tell(&mut self, target: ServiceId, msg: &super::value::Msg) {
        self.0.tell(target, msg);
    }

    /// Checkpoint state and yield to other actors.
    pub fn yield_now(&mut self) -> super::run::Yield {
        self.0.yield_now()
    }
    ...
    /// Queue a new service spawn from a code hash.
    pub fn spawn(&mut self, code_hash: [u8; 32]) {
        self.0.spawn(code_hash);
    }

    /// Store a preimage (code blob, data, etc.) for later retrieval.
    pub fn provide(&mut self, hash: [u8; 32], data: Vec<u8>) {
        self.0.provide(hash, data);
    }

    /// Install a new child service from a code blob and its hash.
    pub fn install(&mut self, hash: [u8; 32], code_blob: Vec<u8>) {
        self.0.install(hash, code_blob);
    }
    ...
    // ── macro-internal methods ──────────────────────────────────────

    #[doc(hidden)]
    pub fn __set_reply(&mut self, value: super::value::Value) {
        self.0.__set_reply(value);
    }

    /// Take the reply as raw bytes (rkyv-encoded Value).
    pub fn take_reply_bytes(&mut self) -> Vec<u8> {
        self.0.take_reply_bytes()
    }
    ...
    pub fn drain_into_refine_payload(
        ...
        self.0.drain_into_refine_payload(state, reply)
    }
}
