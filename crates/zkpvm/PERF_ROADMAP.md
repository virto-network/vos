...
* **Step 8 — PARTIALLY DONE (`af777d6`)**: `ingest_ristretto_boundary` now branches on `rec.kind` — `FixedBasepoint` records route to `ristretto_comb_calls` and skip RistrettoChip's boundary rows; `Variable` records continue through the existing 6-row block.  `populate_ristretto_comb_counts` is called inline (idempotent: zeroes counts before walking).  **Soundness binding deferred**: the consumer chip's per-window `k_i` nibbles are NOT yet tied to the ECALL's input scalar bytes, and the chip's final extended-Edwards Acc is NOT yet tied to the output's compressed Ristretto bytes (compress chain R1e-bis is the next-larger deliverable).  **Production reality**: the current cipher-clerk clerk-private-pay-bench surfaces 7 ECALL_RISTRETTO_SCALAR_MULT records, none of which match `RISTRETTO_BASEPOINT_COMPRESSED.to_bytes()` — so the comb path stays dormant in production traces and the bench is unchanged (~0.74s MOBILE, within noise of the 0.71s baseline).  Investigation needed: why does `&v * &RISTRETTO_BASEPOINT_TABLE` in cipher-clerk's `pvm-precompile` path not surface canonical basepoint bytes?  Possible causes: stale cipher-clerk build, feature-flag mismatch, or a different `RISTRETTO_BASEPOINT_TABLE` const passed in.
