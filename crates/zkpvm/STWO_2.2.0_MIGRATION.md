...
| Validation: `prove_add64` (production path) | ⏳ ConstraintsNotSatisfied | — | OODS still fails after Waves 1-8 (~140 helper cells/row in CpuChip) + Phase 17 sign-source flatten + Phase 31 ValRPartialNZ flatten + 4 grep-missed residuals.  Bug is small-surface but elusive — requires hands-on instrumentation in a fresh session. |
| Validation: `harness_cpuchip_isolated_add64` | ⏳ ConstraintsNotSatisfied (`#[ignore]`'d) | — | Same failure mode as prove_add64.  Use this for bisection: it isolates to CpuChip + minimal prove pipeline.  Likely-suspect helpers ranked by complexity: Wave 3 PartNZTimesIndH M31-arithmetic, Wave 4b TZ/LZ result-binding sums, Wave 7 Phi7TimesInvH field-element product, Wave 8 ValR* Phase 31 helpers, or generate_interaction_trace pair-shape mismatch. |
