...
impl CrdtChangeV2 {
    ...
    pub fn expected_producer(&self) -> Option<ActorId> {
        if let [WorkflowOperationV2::ExpireCall(timeout)] = self.workflow.as_slice() {
...
impl V2Wire for ActorUpgradeV2 {
    ...
    fn decode_body(d: &mut Decoder<'_>) -> Result<Self, DecodeError> {
        ...
        if value.service.execution_semantics != super::EXECUTION_SEMANTICS_ID
            ...
            || super::PackageRolePoliciesV2::decode(&value.role_policies).is_err()
            || !matches!(
                &value.authorization,
...
mod tests {
    ...
    fn call_expiration_wires_bind_the_exact_logical_timeout() {
        ...
        let request = AccumulateRequestV2::ExpireCall(expiration.clone());
        assert_eq!(
            AccumulateRequestV2::decode(&request.encode()).unwrap(),
            request
        );
        ...
        let result = AccumulationResultV2::CallExpired {
            timeout: accumulated,
            duplicate: false,
        };
        assert_eq!(
