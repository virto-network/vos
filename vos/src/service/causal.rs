...
use super::{ActorGenesis, BlobRef, CrdtChange, Hash, ServiceWire};
...
pub(crate) struct CausalFrontier {
    heads: Vec<Hash>,
    nodes: BTreeMap<Hash, CrdtChange>,
    pub max_head_height: u64,
}

impl CausalFrontier {
    /// Return one CID-validated node retained by this frontier.
    pub fn node(&self, cid: Hash) -> Option<&CrdtChange> {
        self.nodes.get(&cid)
    }
    ...
    pub fn nodes_in_causal_order(&self) -> Vec<(Hash, &CrdtChange)> {
        let mut nodes = self
            .nodes
            .iter()
    ...
    ) -> Result<Vec<BlobRef>, CausalSelectionError> {
        if self.heads.is_empty() {
            return Ok(alloc::vec![descriptor.initial_state.clone()]);
        }

        let mut frontier = BTreeMap::<Hash, BlobRef>::new();
        let mut visited = BTreeSet::new();
...
pub(crate) fn load_causal_frontier<E>(
...
) -> Result<CausalFrontier, CausalFrontierError<E>> {
    load_causal_frontier_changes(heads, |cid| {
        ...
        CrdtChange::decode(&bytes).map_err(|_| CausalFrontierError::Corrupt)
    })
}
...
pub(crate) fn load_causal_frontier_with_owned_supplied<E>(
    heads: &[Hash],
    mut supplied: Vec<(Hash, CrdtChange)>,
    mut read_node: impl FnMut(Hash) -> Result<Option<Vec<u8>>, E>,
) -> Result<CausalFrontier, CausalFrontierError<E>> {
    load_causal_frontier_changes(heads, |cid| {
...
fn load_causal_frontier_changes<E>(
    heads: &[Hash],
    mut read_node: impl FnMut(Hash) -> Result<CrdtChange, CausalFrontierError<E>>,
) -> Result<CausalFrontier, CausalFrontierError<E>> {
    let mut nodes = BTreeMap::<Hash, CrdtChange>::new();
    let mut pending = heads.to_vec();
    while let Some(cid) = pending.pop() {
        if nodes.contains_key(&cid) {
            continue;
...
mod tests {
    ...
    use super::*;
    use crate::service::{ActorId, ChangeId, CrdtMaterialization};

    fn change(id: u8, parents: Vec<Hash>, height: u64, state: Option<u8>) -> CrdtChange {
        CrdtChange {
            id: ChangeId([id; 32]),
    ...
    fn validates_complete_ancestry_and_preserves_concurrent_frontier() {
        ...
        let descriptor = ActorGenesis {
            ...
            initial_state: BlobRef::of_bytes(b"initial"),
            crdt: true,
            role_policies: super::super::PackageRolePolicies {
                methods: vec![],
        ...
        assert!(states.contains(&BlobRef::of_bytes(&[2])));
        assert!(states.contains(&BlobRef::of_bytes(&[3])));
