...
use super::{Hash, ServiceWire, StateKey};

pub const SERVICE_STATE_LEAF_DOMAIN: &[u8] = b"vos/service-state-leaf/service";
pub const SERVICE_STATE_NODE_DOMAIN: &[u8] = b"vos/service-state-node/service";
pub const SERVICE_STATE_KEY_DOMAIN: &[u8] = b"vos/service-state-key/service";

const TREE_DEPTH: usize = 256;
const NODE_STORAGE_PREFIX: &[u8] = b"\0vos/service/state-node/";
...
impl<E: core::fmt::Debug> core::fmt::Display for StateTreeError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "invalid VOS service state tree: {self:?}")
    }
}
    ...
    fn decode_body(d: &mut Decoder<'_>) -> Result<Self, DecodeError> {
        match d.u8()? {
            0 => {
                ...
                let logical_key = StateKey::decode(&d.bytes()?)?;
                let value = d.bytes()?;
...
pub struct ServiceStateTree<'a, S: StateTreeStore> {
    store: &'a mut S,
    root: Hash,
}

impl<'a, S: StateTreeStore> ServiceStateTree<'a, S> {
    ...
    pub fn get(&self, key: &StateKey) -> Result<Option<Vec<u8>>, StateTreeError<S::Error>> {
    ...
    fn update(
        ...
        if current == Hash::ZERO {
            return match value {
                Some(value) => self.store_node(&StateNode::Leaf {
                    position: *position,
        ...
        match self.read_node(current)? {
            StateNode::Leaf {
                ...
                let inserted = self.store_node(&StateNode::Leaf {
                    position: *position,
                    logical_key: key.clone(),
                    value: value.to_vec(),
                })?;
    ...
    fn store_branch(
        ...
        self.store_node(&StateNode::Branch {
            bit: bit as u16,
    ...
    fn read_node(&self, expected: Hash) -> Result<StateNode, StateTreeError<S::Error>> {
        let bytes = self
            .store
        ...
        let node = StateNode::decode(&bytes).map_err(|_| StateTreeError::CorruptNode)?;
        let actual = hash_node(&node);
        if actual != expected {
            return Err(match node {
                StateNode::Leaf { .. } => StateTreeError::CorruptLeaf,
                StateNode::Branch { .. } => StateTreeError::CorruptNode,
            });
        }
        Ok(node)
    }

    fn store_node(&mut self, node: &StateNode) -> Result<Hash, StateTreeError<S::Error>> {
...
pub fn state_position(key: &StateKey) -> [u8; 32] {
...
fn hash_node(node: &StateNode) -> Hash {
    match node {
        StateNode::Leaf {
            position,
            logical_key,
...
/// Hash the canonical StateKey wire between an optional prefix and suffix
...
fn hash_state_key(domain: &[u8], prefix: &[&[u8]], key: &StateKey, suffix: &[&[u8]]) -> Hash {
    ...
    let tag = [match key {
        ...
        StateKey::ActorRow { .. } => 2,
        StateKey::Continuation(_) => 3,
        StateKey::Inbox(_) => 4,
        StateKey::Outbox(_) => 5,
        StateKey::Workflow(_) => 6,
        StateKey::CrdtMaterialization(_) => 7,
        StateKey::PendingAuthorizedInbox(_) => 8,
        StateKey::ActorDirectory => 9,
        StateKey::ExternalActorDirectory => 10,
        StateKey::RoleAuthority => 11,
    }];
    let length = match key {
        StateKey::MethodPolicy { method, .. } => (method.len() as u32).to_le_bytes(),
        StateKey::ActorRow { key, .. } => (key.len() as u32).to_le_bytes(),
        _ => [0; 4],
    };
    ...
    match key {
        ...
        StateKey::Inbox(call) | StateKey::Outbox(call) => {
            parts[used] = &call.0;
            used += 1;
        }
        ...
        StateKey::MethodPolicy { actor, method } => {
            for part in [&actor.0[..], &length, method.as_bytes()] {
                parts[used] = part;
                used += 1;
            }
        }
        StateKey::ActorRow { actor, key } => {
            for part in [&actor.0[..], &length, key.as_slice()] {
                parts[used] = part;
                used += 1;
            }
        }
        StateKey::ActorDirectory | StateKey::ExternalActorDirectory | StateKey::RoleAuthority => {}
    }
    for part in suffix {
        parts[used] = part;
        used += 1;
...
mod tests {
    ...
    use super::*;
    use crate::service::{ActorId, CallId, InvocationId};
    ...
    fn rebuild(values: &BTreeMap<StateKey, Vec<u8>>) -> Hash {
        let mut store = MemStore::default();
        let mut root = empty_state_root();
        for (key, value) in values {
            root = ServiceStateTree::new(&mut store, root)
    ...
    fn allocation_free_key_hashing_matches_the_canonical_wire() {
        ...
        let keys = vec![
            ...
            StateKey::Continuation(actor),
            StateKey::Inbox(CallId([8; 32])),
            StateKey::Outbox(CallId([9; 32])),
            StateKey::Workflow(InvocationId([10; 32])),
            StateKey::CrdtMaterialization(actor),
            StateKey::PendingAuthorizedInbox(actor),
        ];
        let position = [11; 32];
    ...
    fn incremental_updates_match_canonical_recomputation() {
        ...
        for (key, value) in expected {
            let tree = ServiceStateTree::new(&mut store, root);
    ...
    fn insertion_order_does_not_change_the_root() {
        ...
        for (key, value) in &entries {
            forward = ServiceStateTree::new(&mut forward_store, forward)
                .apply(key, Some(value))
                .unwrap();
        }
        ...
        for (key, value) in entries.iter().rev() {
            reverse = ServiceStateTree::new(&mut reverse_store, reverse)
                .apply(key, Some(value))
                .unwrap();
        }
    ...
    fn failed_staging_transaction_never_changes_committed_root() {
        ...
        let result = ServiceStateTree::new(&mut staging, root).apply(&row(1), Some(b"value"));
