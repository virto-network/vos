...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageTaskDependency {
    pub binding: TaskDependency,
    pub pvm: Vec<u8>,
}

/// Canonical generated authorization artifact carried by `.vos` service.
///
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PackageRolePolicies {
...
impl core::fmt::Display for PackageError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "invalid .vos service package: {self:?}")
    }
}
...
impl VosPackage {
    pub fn validate(&self) -> Result<(), PackageError> {
        ...
        let expected = PackageRolePolicies::from_metadata(&metadata)?;
    ...
    pub fn actor_genesis(
        ...
        initial_state: BlobRef,
    ) -> Result<ActorGenesis, PackageError> {
        self.validate()?;
...
pub fn method_schema_hash(message: &ParsedMessage) -> Hash {
    ...
    Hash::digest(b"vos/method-schema/service", &[&bytes])
}
...
pub fn public_policy_hash() -> Hash {
    Hash::digest(b"vos/public-policy/service", &[])
}
...
pub fn artifact_hash(kind: &[u8], bytes: &[u8]) -> Hash {
    Hash(crate::crypto::blake2b_hash::<32>(
        b"vos/package-artifact/service",
        &[kind, bytes],
    ))
}
...
pub fn task_dependencies_hash(dependencies: &[PackageTaskDependency]) -> Hash {
    let mut bytes = Vec::new();
    let mut encoder = Encoder(&mut bytes);
...
impl ServiceWire for VosPackage {
...
impl ServiceWire for PackageRolePolicies {
    ...
    fn decode_body(decoder: &mut Decoder<'_>) -> Result<Self, DecodeError> {
        let methods = decoder.list(|decoder| MethodPolicy::decode(&decoder.bytes()?))?;
        if methods
            .windows(2)
            .any(|pair| pair[0].method >= pair[1].method)
        {
...
fn decode_task_dependencies(decoder: &mut Decoder<'_>) -> Result<Vec<TaskDependency>, DecodeError> {
...
fn decode_package_task_dependencies(
...
) -> Result<Vec<PackageTaskDependency>, DecodeError> {
    let len = decoder.u32()? as usize;
    if len > super::MAX_PACKAGE_TASK_DEPENDENCIES {
        return Err(DecodeError::LimitExceeded);
    }
    let mut dependencies = Vec::with_capacity(len);
    for _ in 0..len {
        dependencies.push(PackageTaskDependency {
...
fn encode_task_dependency(encoder: &mut Encoder<'_>, dependency: &TaskDependency) {
...
fn decode_task_dependency(decoder: &mut Decoder<'_>) -> Result<TaskDependency, DecodeError> {
...
mod tests {
    ...
    fn package_roundtrip_is_deterministic() {
        ...
        let decoded = VosPackage::decode(&bytes).unwrap();
    ...
    fn task_dependencies_are_signed_canonical_package_content() {
        ...
        package.validate().unwrap();
        let decoded = VosPackage::decode(&package.encode()).unwrap();
    ...
    fn method_policies_are_derived_from_schema_and_annotations() {
        let package = package();
        let policies = PackageRolePolicies::decode(&package.role_policies).unwrap();
        ...
        let actor_local = PackageRolePolicies::from_metadata(&metadata).unwrap();
    ...
    fn guest_install_descriptor_uses_only_signed_package_policies() {
        ...
        let state = BlobRef::of_bytes(b"initial state");
        let genesis = package
        ...
        assert_eq!(
            PackageRolePolicies::decode(&genesis.role_policies).unwrap(),
            policies
        );
        assert_eq!(
