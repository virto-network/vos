//! VOS binding envelope for a canonical PVM invocation snapshot.
//!
//! The PVM owns the portable machine format and restore semantics. VOS does
//! not mirror PC, registers, capabilities, memory, or the nested call stack in
//! a second structure. It binds the exact PVM snapshot
...
pub struct ContinuationSnapshot {
    ...
    /// Exact actor VM which issued `pending_call`. Derived from PVM's active
    ...
    /// Exact actors whose machines are running or waiting on the nested PVM
...
/// Allocation-bounded view used by guest Accumulate. The inner PVM snapshot
...
impl ContinuationSnapshot {
    ...
    /// Check the workflow binding before the PVM restores the inner
    ...
    pub(crate) fn decode_metadata(bytes: &[u8]) -> Result<ContinuationMetadata, DecodeError> {
        ...
        if super::Hash(d.fixed()?) != super::PLATFORM_ID {
            return Err(DecodeError::InvalidPlatform);
        }
...
fn decode_service(d: &mut Decoder<'_>) -> Result<ServiceIdentity, DecodeError> {
    let service = ServiceIdentity {
        ...
        platform: super::Hash(d.fixed()?),
        execution_semantics: super::Hash(d.fixed()?),
        gas_schedule: super::GasSchedule::new(d.u64()?, d.u64()?),
    };
    if service.platform != super::PLATFORM_ID || !service.gas_schedule.is_valid() {
        return Err(DecodeError::InvalidPlatform);
    }
    Ok(service)
}
...
mod tests {
    ...
    fn snapshot() -> ContinuationSnapshot {
        ...
        ContinuationSnapshot {
            ...
            kernel_snapshot: b"canonical PVM kernel snapshot".to_vec(),
        }
    }
