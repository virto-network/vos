...
use super::{
    ...
    ACTOR_PRIVATE_INPUT_MAX_BYTES, AccumulatedRoleAssertion, AccumulationResult, ActorEffectBatch,
...
pub const SERVICE_ARGUMENT_PAGES_: u32 = 2048;

/// Transpile the protocol infrastructure ELF with the service standard argument
...
pub fn transpile_service_elf(elf: &[u8]) -> Result<Vec<u8>, vos_pvm_compiler::TranspileError> {
    vos_pvm_compiler::link_elf_with_argument_pages(elf, SERVICE_ARGUMENT_PAGES_)
}

/// Result of one completed service-PVM execution slice.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ServicePvmOutput {
    pub bytes: Vec<u8>,
    pub gas_used: u64,
    ...
    pub producer_records: Vec<ProducedProvableRecord>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ProducedProvableRecord {
...
pub enum ServicePvmError {
    ...
    ActorStorageWitnessRequired(Vec<ActorStorageKey>),
    InvalidVmLifecycle,
}

struct RefineTraceRecorder {
...
impl RefineTraceRecorder {
    fn new(service_program: ProgramId, work: &WorkEnvelope, imports: &RefineImports) -> Self {
        ...
        recorder.bytes(b"vos/refine-trace/service");
        recorder.bytes(&super::EXECUTION_SEMANTICS_ID.0);
        recorder.bytes(&service_program.0);
        recorder.bytes(&work.hash().0);
        recorder.bytes(&Hash::digest(b"vos/refine-imports/service", &[&imports.encode()]).0);
        recorder
    }
    ...
    fn checkpoint(&mut self, artifact: &ImportedBlob) {
        self.state.update(&[3]);
    ...
    fn task_begin(
        ...
        self.bytes(&actor.0);
        self.bytes(&task.0);
        self.bytes(&Hash::digest(b"vos/task-input/service", &[input]).0);
        match tag {
            Some(tag) => {
                self.state.update(&[1]);
                self.bytes(&tag);
    ...
    fn output(&mut self, bytes: &[u8], gas_used: u64, exported_blobs: &[ImportedBlob]) {
        self.state.update(&[6]);
        self.bytes(bytes);
        self.u64(gas_used);
...
impl core::fmt::Display for ServicePvmError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "VOS service PVM failed: {self:?}")
    }
}

impl core::error::Error for ServicePvmError {}
...
/// of them; only [`AccumulateProtocolHost::commit`] may make them visible.
pub trait AccumulateTransaction {
    fn handle(
        &mut self,
        slot: u8,
        registers: &[u64; 13],
...
pub trait AccumulateProtocolHost {
    type Transaction: AccumulateTransaction;
    ...
    fn verify_logical_timeslot(&self, _logical_timeslot: u64) -> Result<(), ServicePvmError> {
        Ok(())
    }
...
/// executing IC-5.
pub trait ReceiptVerificationHost {
    fn make_receipt_available(&mut self, request: &super::ReceiptVerificationRequest) -> bool;
}

/// Host used by pure service programs that need no protocol imports.
#[derive(Debug, Clone, Copy, Default)]
pub struct NoRefineProtocolHost;

impl RefineProtocolHost for NoRefineProtocolHost {
    fn handle(
        &self,
        slot: u8,
        _registers: &[u64; 13],
        ...
        Err(ServicePvmError::RefineHostRejected(slot))
    }
}
...
/// drop. It is process configuration only: no Service wire encoder exists, so it
...
impl DeviceSignerRefineHost {
    pub fn new(secret: Option<DeviceSecret>) -> Self {
        ...
        let nonce_key = Hash::digest(b"vos/device-signer-nonce-key/service", &[secret.seed()]).0;
...
fn derive_device_signer(secret: &DeviceSecret) -> cipher_clerk::crypto::SecretKey {
    (0u32..)
        .find_map(|counter| {
            let candidate = Hash::digest(
                b"vos/device-signer-key/service",
...
impl core::fmt::Debug for DeviceSignerRefineHost {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
...
impl RefineProtocolHost for DeviceSignerRefineHost {
    fn handle(
        &self,
        slot: u8,
        registers: &[u64; 13],
        ...
        let nonce_seed = Hash::digest(
            b"vos/device-signer-nonce/service",
            &[
                &self
                    .nonce_key
                    .ok_or(ServicePvmError::RefineHostRejected(slot))?,
...
struct ActorRefineRuntime {
    ...
    resume_work: Vec<u8>,
    program_layout: Vec<super::ContinuationProgram>,
    actor_by_vm: Vec<Option<super::ActorId>>,
    private_inputs: BTreeMap<super::ActorId, ActorPrivateInput>,
    task_programs: BTreeMap<(super::ActorId, Hash), TaskProgram>,
    ...
    missing_storage_rows: BTreeSet<ActorStorageKey>,
    producer_records: Vec<ProducedProvableRecord>,
    producer_record_bytes: usize,
    outputs: Vec<ActorSliceOutput>,
...
impl ActorRefineRuntime {
    fn new(
        work: &WorkEnvelope,
        imports: &RefineImports,
        space_role: Option<u8>,
        actor_role: Option<u8>,
        captured_programs: Option<&[super::ContinuationProgram]>,
    ) -> Result<Self, ServicePvmError> {
        let program_layout = captured_programs.map_or_else(
            || {
                work.imported_actors
                    .iter()
                    .map(|actor| super::ContinuationProgram {
                        actor: actor.actor,
            ...
            <[super::ContinuationProgram]>::to_vec,
        );
        ...
        for actor in work.imported_actors.iter().filter(|actor| {
            ...
            if private.encode().len() > ACTOR_PRIVATE_INPUT_MAX_BYTES {
                return Err(ServicePvmError::ActorInputTooLarge);
            }
            ...
            for row in &actor.storage_rows {
                ...
                if storage_rows
                    ...
                    .is_some()
                {
                    return Err(ServicePvmError::InvalidRefineImports);
                }
            }
            for dependency in &actor.task_dependencies {
                let imported = imports
                    ...
                    .ok_or(ServicePvmError::InvalidRefineImports)?;
        ...
        Ok(Self {
            ...
            outputs: Vec::new(),
            // Service header plus the effect-batch list length.
            encoded_outputs_len: 10,
        })
    }

    fn take_producer_records(&mut self) -> Vec<ProducedProvableRecord> {
        core::mem::take(&mut self.producer_records)
    }

    fn take_missing_storage_rows(&mut self) -> Vec<ActorStorageKey> {
    ...
    fn write_task_result(
        ...
        if !kernel.write_data_cap_window(address, bytes) {
            return Err(ServicePvmError::RefineHostRejected(
    ...
    fn handle_task_invoke(
        &mut self,
        kernel: &mut InvocationKernel,
        mut trace: Option<&mut RefineTraceRecorder>,
    ) -> Result<[u64; 2], ServicePvmError> {
        ...
        let hash_address = u32::try_from(kernel.active_reg(7))
            .map_err(|_| ServicePvmError::RefineHostRejected(crate::abi::hostcall::INVOKE as u8))?;
        let input_address = u32::try_from(kernel.active_reg(8))
            .map_err(|_| ServicePvmError::RefineHostRejected(crate::abi::hostcall::INVOKE as u8))?;
        let input_len = u32::try_from(kernel.active_reg(9))
            .map_err(|_| ServicePvmError::RefineHostRejected(crate::abi::hostcall::INVOKE as u8))?;
        ...
        let failure = loop {
            let result = if let Some(recorder) = trace.as_deref_mut() {
                child
                    ...
                    .map_err(|_| ServicePvmError::TraceBackendRequired)?
            } else {
                child.run()
            };
            match result {
        ...
        let output_address = u32::try_from(child.active_reg(7))
            .map_err(|_| ServicePvmError::RefineHostRejected(crate::abi::hostcall::INVOKE as u8))?;
        ...
        if let Some(tag) = tag {
            ...
            self.producer_records
                .push(ProducedProvableRecord { actor, tag, entry });
    ...
    fn handle(
        ...
        slot: u8,
        kernel: &mut InvocationKernel,
        trace: Option<&mut RefineTraceRecorder>,
    ) -> Result<Option<[u64; 2]>, ServicePvmError> {
        match slot as u32 {
                ...
                let Some(tag) = key
                    ...
                    let Some(value) = self.storage_rows.get(&lookup) else {
                        ...
                        self.missing_storage_rows
                            .insert(ActorStorageKey { actor, key });
            ...
            crate::abi::hostcall::ACTOR_EFFECT_EXPORT => {
                ...
                let bytes = kernel
                    ...
                    .ok_or(ServicePvmError::RefineHostRejected(slot))?;
                let mut output = ActorSliceOutput::decode(&bytes)
                    .map_err(|_| ServicePvmError::RefineHostRejected(slot))?;
                ...
                match private.change {
                    ...
                    Some(dispatch) => {
                        ...
                        private.change = Some(CrdtDispatch {
...
impl ServicePvm {
    pub fn new(program: Vec<u8>, expected: ProgramId) -> Result<Self, ServicePvmError> {
        validate_service_entries(&program)?;
        let actual = ProgramId::of_pvm(&program);
        if actual != expected {
            return Err(ServicePvmError::ProgramIdMismatch);
        }
        Ok(Self {
            program,
            program_id: actual,
    ...
    pub fn refine<H: RefineProtocolHost>(
        &self,
        arguments: &[u8],
        gas_limit: u64,
        host: &H,
    ) -> Result<ServicePvmOutput, ServicePvmError> {
    ...
    pub fn refine_actor_tree_traced<H: RefineProtocolHost>(
        &self,
        arguments: &[u8],
        imports: &RefineImports,
        gas_limit: u64,
    ...
    fn refine_actor_tree_internal<H: RefineProtocolHost>(
        &self,
        arguments: &[u8],
        imports: &RefineImports,
        gas_limit: u64,
        ...
        let work =
            WorkEnvelope::decode(arguments).map_err(|_| ServicePvmError::InvalidWorkEnvelope)?;
        imports
            .validate_for(&work)
            .map_err(|_| ServicePvmError::InvalidRefineImports)?;
        if work.imported_actors.len() > MAX_ROOT_TREE_ACTORS {
            return Err(ServicePvmError::TooManyImportedActors);
        }
        let trace = traced.then(|| RefineTraceRecorder::new(self.program_id, &work, imports));
        ...
        for (ordinal, actor) in actors.into_iter().enumerate() {
            ...
            let handle_slot = TARGET_ACTOR_HANDLE_SLOT
                ...
                .ok_or(ServicePvmError::TooManyImportedActors)?;
        ...
        if let Some(reference) = target.continuation.as_ref() {
            ...
            let continuation = ContinuationSnapshot::decode(bytes)
                .map_err(|_| ServicePvmError::InvalidContinuation)?;
            continuation
                .validate_resume_for(&work)
                .map_err(|_| ServicePvmError::ContinuationMismatch)?;
            let actor_runtime = ActorRefineRuntime::new(
                &work,
                imports,
                space_role,
                actor_role,
            ...
            let target_binding = continuation
                ...
                .ok_or(ServicePvmError::ContinuationMismatch)?;
            ...
            let (resume_kind, payload_len) = match (
                ...
                (Some(call), Some(awaited), None) if awaited.reply.call_id == call => {
                    let attestation = awaited
                        ...
                        .map(|attestation| {
                            ...
                            Ok(alloc::boxed::Box::new(super::AttestationResume {
                ...
                _ => return Err(ServicePvmError::ContinuationMismatch),
            };
            kernel
                .resume_protocol_call(resume_kind, payload_len)
                .map_err(|_| ServicePvmError::InvalidProtocolResume)?;
        ...
        let actor_input = ActorSliceInput {
            actor: work.target,
        ...
        let actor_runtime = ActorRefineRuntime::new(&work, imports, space_role, actor_role, None)?;
    ...
    pub fn accumulate<H: AccumulateProtocolHost>(
        &self,
        arguments: &[u8],
        gas_limit: u64,
        host: &mut H,
    ...
    pub fn accumulate_with_availability<H: AccumulateProtocolHost>(
        &self,
        arguments: &[u8],
        gas_limit: u64,
        host: &mut H,
    ...
    pub fn accumulate_at_with_availability<H: AccumulateProtocolHost>(
        &self,
        arguments: &[u8],
        gas_limit: u64,
        host: &mut H,
    ...
    fn accumulate_at_with_backend<H: AccumulateProtocolHost>(
        &self,
        arguments: &[u8],
        gas_limit: u64,
        host: &mut H,
        ...
        kernel
            ...
            .map_err(|_| ServicePvmError::InvalidVmLifecycle)?;
        ...
        loop {
            match kernel.run() {
                KernelResult::Halt => {
                    ...
                    let result = AccumulationResult::decode(&bytes)
                        .map_err(|_| ServicePvmError::InvalidAccumulateOutput)?;
                    if matches!(
                        result,
                ...
                KernelResult::Panic => {
                    return Err(ServicePvmError::Panic {
                        vm: kernel.active_vm,
                        pc: kernel.vm_arena.vm(kernel.active_vm).pc,
                    });
                }
                KernelResult::OutOfGas => {
                    return Err(ServicePvmError::OutOfGas {
                        vm: kernel.active_vm,
                        pc: kernel.vm_arena.vm(kernel.active_vm).pc,
                    });
                }
                KernelResult::PageFault(address) => {
                    return Err(ServicePvmError::PageFault {
                        vm: kernel.active_vm,
                        address,
                    });
                }
...
fn map_kernel_initialization_error(error: vos_pvm::kernel::KernelError) -> ServicePvmError {
    match error {
        vos_pvm::kernel::KernelError::MemoryError
        | vos_pvm::kernel::KernelError::OutOfMemory
        | vos_pvm::kernel::KernelError::CompileError => ServicePvmError::KernelResourceUnavailable,
        ...
        | vos_pvm::kernel::KernelError::TooManyVms => ServicePvmError::InvalidProgram,
    }
}
...
fn actor_tree_from_work(work: &WorkEnvelope) -> Vec<ActorTreeImport> {
    work.imported_actors
        .iter()
        .map(|actor| ActorTreeImport {
            actor: actor.actor,
...
fn continued_actor_vm_index(
    programs: &[super::ContinuationProgram],
    target: super::ActorId,
    actor: super::ActorId,
) -> Option<u16> {
    if actor == target {
    ...
    for destination in programs {
        ...
        for (source_index, source_binding) in programs.iter().enumerate() {
            ...
            let (vm_id, max_gas) = match kernel.vm_arena.vm(0).cap_table.get(source_handle) {
                ...
                _ => return Err(ServicePvmError::InvalidContinuation),
            };
...
fn install_actor_ipc(
    ...
    {
        return Err(ServicePvmError::ActorIpcExhausted);
    }
    ...
    if !kernel.backing.write_init_data(backing_offset, input) {
        return Err(ServicePvmError::ActorIpcSetupFailed);
    }
...
fn crdt_dispatch(work: &WorkEnvelope, ordinal: u32) -> Option<CrdtDispatch> {
    CrdtChange::derive_operation_scope(work).map(|change| CrdtDispatch { change, ordinal })
}

fn imported_blob_bytes<'a>(
    imports: &'a RefineImports,
    reference: &BlobRef,
) -> Result<&'a [u8], ServicePvmError> {
    imports
        .blobs
...
fn authorization_roles(
    work: &WorkEnvelope,
    imports: &RefineImports,
) -> Result<(Option<u8>, Option<u8>), ServicePvmError> {
    let (commitment, bytes) = match &work.authorization {
        AuthorizationEvidence::Public | AuthorizationEvidence::SystemCapability { .. } => {
            return Ok((None, None));
        }
        AuthorizationEvidence::Credential {
            ...
            let bytes = if bytes.is_empty()
                && work.consistency == super::ConsistencyMode::Crdt
                && work.parent_call.is_none()
            {
                imports
                    ...
                    .ok_or(ServicePvmError::InvalidAuthorization)?
            } else {
                bytes.as_slice()
            };
    ...
    let credential =
        RoleCredential::decode(bytes).map_err(|_| ServicePvmError::InvalidAuthorization)?;
    ...
    if credential.holder != work.origin
        ...
        || commitment != Hash::digest(b"vos/credential-commitment/service", &[bytes])
        ...
        || (credential.actor_role.is_some()
            && AccumulatedRoleAssertion::decode(&credential.authenticator).is_ok())
    {
        return Err(ServicePvmError::InvalidAuthorization);
    }
    Ok((
...
fn stage_attestation_proof(
    ...
    let capacity = match kernel.vm_arena.vm(kernel.active_vm).cap_table.get(0) {
        ...
        _ => return Err(ServicePvmError::ActorIpcSetupFailed),
    };
    if proof.is_empty()
    ...
    let address = u32::try_from(address).map_err(|_| ServicePvmError::ActorIpcExhausted)?;
    if !kernel.write_data_cap_window(address, proof) {
        return Err(ServicePvmError::ActorIpcSetupFailed);
    }
    Ok((
        u32::try_from(offset).map_err(|_| ServicePvmError::ActorIpcExhausted)?,
        u32::try_from(proof.len()).map_err(|_| ServicePvmError::ActorIpcExhausted)?,
    ))
}
...
fn write_suspension_payload(
    ...
    u64::try_from(encoded.len()).map_err(|_| ServicePvmError::CheckpointTokenWriteFailed)
}

fn capture_checkpoint(
    kernel: &mut InvocationKernel,
    work: &WorkEnvelope,
    actor_runtime: &ActorRefineRuntime,
) -> Result<
    (
        ImportedBlob,
        KernelSnapshot,
    ...
    let pending_actor = awaited
        .then(|| {
            actor_runtime
                ...
                .ok_or(ServicePvmError::SnapshotFailed)
        })
        .transpose()?;
...
fn suspended_actor_stack(
    ...
    actor_runtime: &ActorRefineRuntime,
) -> Result<Vec<super::ActorId>, ServicePvmError> {
...
fn run_refine_kernel<H: RefineProtocolHost>(
    mut kernel: InvocationKernel,
    host: &H,
    backend: vos_pvm::PvmBackend,
    fresh: bool,
    suspension_work: Option<&WorkEnvelope>,
    invocation_layout: Option<(&[u8], &[DormantProgram<'_>])>,
    mut actor_runtime: Option<ActorRefineRuntime>,
    previously_suspended: Vec<super::ActorId>,
    mut exported_blobs: Vec<ImportedBlob>,
    mut trace: Option<RefineTraceRecorder>,
) -> Result<ServicePvmOutput, ServicePvmError> {
    if fresh {
        kernel
            .vm_arena
        ...
        match result {
            KernelResult::Halt => {
                if let Some(runtime) = actor_runtime.as_mut() {
                    ...
                    if !missing.is_empty() {
                        return Err(ServicePvmError::ActorStorageWitnessRequired(missing));
                    }
                }
                let bytes = read_output(&kernel)?;
                ...
                let producer_records = actor_runtime
                    .as_mut()
                    .map(ActorRefineRuntime::take_producer_records)
                    .unwrap_or_default();
            ...
            KernelResult::ProtocolCall { slot } => {
                ...
                if !refine_protocol_call_is_pure(slot) {
                    return Err(ServicePvmError::ForbiddenRefineProtocolCall(slot));
                }
                if slot == crate::abi::hostcall::SUSPEND as u8 {
                    if let Some(work) = suspension_work {
                        ...
                        let (service_program, dormant) =
                            invocation_layout.ok_or(ServicePvmError::InvalidContinuation)?;
...
fn validate_service_entries(program: &[u8]) -> Result<(), ServicePvmError> {
    let parsed = parse_blob(program).ok_or(ServicePvmError::InvalidProgram)?;
    let argument_cap = parsed
        .caps
        .iter()
        .find(|cap| cap.cap_index == 0 && cap.cap_type == CapEntryType::Data)
        .ok_or(ServicePvmError::InvalidServiceEntries)?;
    if argument_cap.page_count < SERVICE_ARGUMENT_PAGES_ {
        return Err(ServicePvmError::InvalidServiceEntries);
    }
    let code_cap = parsed
        .caps
        .iter()
        .find(|cap| cap.cap_index == parsed.header.invoke_cap && cap.cap_type == CapEntryType::Code)
        .ok_or(ServicePvmError::InvalidProgram)?;
...
pub fn validate_actor_program_layout(program: &[u8]) -> Result<(), ServicePvmError> {
    let parsed = parse_blob(program).ok_or(ServicePvmError::InvalidProgram)?;
    let callable_end = super::ACTOR_CALLABLE_BASE_SLOT
        .checked_add(MAX_ROOT_TREE_ACTORS as u8)
        .ok_or(ServicePvmError::InvalidActorCapabilityLayout)?;
...
fn read_output(kernel: &InvocationKernel) -> Result<Vec<u8>, ServicePvmError> {
    let address =
        u32::try_from(kernel.active_reg(7)).map_err(|_| ServicePvmError::UnreadableOutput)?;
    let len = u32::try_from(kernel.active_reg(8)).map_err(|_| ServicePvmError::UnreadableOutput)?;
    kernel
        .read_data_cap_window(address, len)
        .ok_or(ServicePvmError::UnreadableOutput)
}
...
mod tests {
    ...
    fn device_signer_seed_derivation_is_stable_and_redacted() {
        let seed = [0x5a; 32];
        let left = DeviceSignerRefineHost::new(Some(DeviceSecret::new(seed)));
        let right = DeviceSignerRefineHost::new(Some(DeviceSecret::new(seed)));
        let other = DeviceSignerRefineHost::new(Some(DeviceSecret::new([0xa5; 32])));

        assert_eq!(left.public_key(), right.public_key());
        assert_eq!(
            left.public_key(),
            Some(DeviceSecret::new(seed).public_key())
        );
        assert_ne!(left.public_key(), other.public_key());
        assert_ne!(left.public_key(), Some([0; 32]));
        assert_eq!(
            format!("{:?}", DeviceSecret::new(seed)),
            "DeviceSecret(<redacted>)"
        );
        assert!(format!("{left:?}").starts_with("DeviceSignerRefineHost { configured: true"));
    }
    ...
    fn disclosed_and_private_role_credentials_feed_the_same_actor_role() {
        ...
        assert_eq!(
            authorization_roles(&work, &RefineImports::default()),
            Ok((Some(crate::SpaceRole::Developer.as_u8()), Some(2)))
        );
        work.arguments = vec![9];
        assert_eq!(
            authorization_roles(&work, &RefineImports::default()),
            Err(ServicePvmError::InvalidAuthorization),
        ...
        let authority_service = super::super::ServiceIdentity {
        ...
        let assertion = AccumulatedRoleAssertion {
            receipt: super::super::AccumulationReceipt {
    ...
    impl AccumulateTransaction for RecordingTransaction {
        fn handle(
            &mut self,
            slot: u8,
            _registers: &[u64; 13],
    ...
    impl AccumulateProtocolHost for RecordingAccumulateHost {
    ...
    fn accumulate_result(commit: bool) -> Vec<u8> {
        use crate::service::{
            AccumulationReceipt, ConsistencyMode, DeploymentId, Hash, RootServiceId,
            ServiceIdentity,
        };

        let mut receipt = AccumulationReceipt {
        ...
        if commit {
            AccumulationResult::Accepted {
                receipt,
                published: crate::service::PublishedEffects::default(),
                duplicate: false,
            }
        } else {
            let actor = crate::service::ActorId([6; 32]);
            let invocation = crate::service::InvocationId([9; 32]);
            let reply = crate::service::ReplyRecord {
            ...
            AccumulationResult::Prepared(crate::attestation::AttestationPreparation {
                receipt,
                statement,
    ...
    fn physical_refine_entry_is_deterministic_and_uses_gp_arguments() {
        ...
        let service = ServicePvm::new(program.clone(), ProgramId::of_pvm(&program)).unwrap();
        let first = service
            .refine(
                b"work-envelope",
    ...
    fn refine_rejects_persistent_protocol_calls_before_host_dispatch() {
        ...
        let service = ServicePvm::new(program.clone(), ProgramId::of_pvm(&program)).unwrap();
        assert_eq!(
            service.refine(&[], SYNTHETIC_SERVICE_GAS, &NoRefineProtocolHost),
            Err(ServicePvmError::ForbiddenRefineProtocolCall(
    ...
    fn service_identity_and_both_physical_entries_are_mandatory() {
        let program = two_entry_program(None);
        assert!(matches!(
            ServicePvm::new(program.clone(), ProgramId([0; 32])),
            Err(ServicePvmError::ProgramIdMismatch)
        ));

        let actor = vos_pvm_compiler::assembler::Assembler::new().build();
        assert!(matches!(
            ServicePvm::new(actor.clone(), ProgramId::of_pvm(&actor)),
            Err(ServicePvmError::InvalidServiceEntries)
        ));
    }
    ...
    fn actor_manifests_cannot_occupy_scheduler_capability_slots() {
        ...
        assert_eq!(
            ...
            Err(ServicePvmError::InvalidActorCapabilityLayout)
        );
    }

    #[test]
    fn service_rejects_an_argument_window_too_small_for_service_wires() {
        ...
        assert!(matches!(
            ServicePvm::new(program.clone(), ProgramId::of_pvm(&program)),
            Err(ServicePvmError::InvalidServiceEntries)
        ));
    }
    ...
    fn accumulate_discards_staging_for_prepared_and_rejected_results() {
        ...
        let rejected =
            AccumulationResult::Rejected(crate::service::AccumulationRejection::Unauthorized)
                .encode();
        assert_eq!(
            service
    ...
    fn accumulate_discards_staging_on_panic_or_commit_failure() {
        ...
        let service = ServicePvm::new(panicking.clone(), ProgramId::of_pvm(&panicking)).unwrap();
        let mut host = RecordingAccumulateHost::default();
        assert!(matches!(
            service.accumulate(&[], SYNTHETIC_SERVICE_GAS, &mut host),
            Err(ServicePvmError::Panic { vm: 0, .. })
        ));
        assert_eq!(host.committed_calls, 0);

        let committing = service_program(None, Some(crate::abi::hostcall::STORAGE_W), false);
        let service = ServicePvm::new(committing.clone(), ProgramId::of_pvm(&committing)).unwrap();
