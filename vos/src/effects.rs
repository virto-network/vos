...
/// Result: `[RESULT_OK][reply…]` or `[RESULT_ERR][invoke_status]`, preserving
/// both unit replies and the target's typed failure status.
...
/// Response status byte: the op failed; the next byte identifies the actor
/// invocation failure.
pub const RESULT_ERR: u8 = 0;

/// Status framing for [`EFFECT_ASK`].
pub mod result {
    use super::{RESULT_ERR, RESULT_OK};
    use alloc::vec::Vec;

    pub fn ok(bytes: &[u8]) -> Vec<u8> {
        let mut o = Vec::with_capacity(1 + bytes.len());
        o.push(RESULT_OK);
        o.extend_from_slice(bytes);
        o
    }
    pub fn err(status: u8) -> Vec<u8> {
        alloc::vec![RESULT_ERR, status]
    }

    pub fn decode(resp: &[u8]) -> Result<Vec<u8>, u8> {
        match resp {
            [RESULT_OK, reply @ ..] => Ok(reply.to_vec()),
            [RESULT_ERR, status, ..] => Err(*status),
            _ => Err(u8::MAX),
        }
    }
}
...
mod tests {
    ...
    fn status_result_roundtrips() {
        assert_eq!(
            super::result::decode(&super::result::ok(b"data")),
            Ok(b"data".to_vec())
        );
        assert_eq!(
            super::result::decode(&super::result::ok(b"")),
            Ok(Vec::new())
        );
        assert_eq!(
            super::result::decode(&super::result::err(crate::STATUS_FORBIDDEN)),
            Err(crate::STATUS_FORBIDDEN),
        );
    }
}
