...
use super::genesis::{
    AgentGenesisArchiveRecord, AgentGenesisLocator, AgentGenesisProposal, AgentGenesisProvider,
    AgentGenesisProviderError as Error, AgentGenesisProvision,
...
impl<S: AgentGenesisArchiveStore> ArchivedAgentGenesisProvider<S> {
    ...
    pub fn load_record(
        &self,
        locator: AgentGenesisLocator,
    ) -> Result<Option<AgentGenesisArchiveRecord>, Error> {
    ...
    pub fn publish(&self, record: &AgentGenesisArchiveRecord) -> Result<(), Error> {
        ...
        self.store
            .insert_if_absent(locator, &bytes)
...
impl<S: AgentGenesisArchiveStore> AgentGenesisProvider for ArchivedAgentGenesisProvider<S> {
    ...
    fn reproduce(&self, locator: AgentGenesisLocator) -> Result<AgentGenesisProvision, Error> {
        self.load_record(locator)?
            .map(|record| record.provision().clone())
            .ok_or(Error::NotConfigured)
    }
