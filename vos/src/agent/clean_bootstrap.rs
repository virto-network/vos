...
impl AgentGenesisFinalityVerifier for ReplayVerifiedAgentGenesisFinalitySet {
    fn verify_finalized(
        &self,
        provision: &super::genesis::AgentGenesisProvision,
    ) -> Result<(), super::genesis::AgentGenesisFinalityError> {
        self.0
            .iter()
            .find(|proof| proof.0.proposal().locator() == provision.proposal().locator())
...
impl AgentGenesisFinalityVerifier for ReplayVerifiedAgentGenesisFinality {
    fn verify_finalized(
        ...
        if provision != &self.0 {
            return Err(super::genesis::AgentGenesisFinalityError::Conflict);
        }
        Ok(())
    }
}
    ...
    I: CleanManagementIssuerStore,
{
    ...
    pub(crate) fn resume_shared_genesis_preparation<B, J, Q, ReplyStore, W, PubReply, S>(
        ...
        (
            ...
            super::committee::AuthorityCommittee,
        ),
    ...
    pub(crate) fn endorse_recovered_shared_genesis<
        B,
        J,
        Q,
        ReplyStore,
    ...
    >(
        ...
        (
            AuthorizedSharedGenesisProposal,
            super::committee::AuthorityCommittee,
            super::committee::AuthoritySignature,
        ),
        SharedAgentHostError,
    >
    where
        ...
        K: genesis_issuance::GenesisClaimSigner,
        W: CleanManagementIssuerStore,
        PubReply: CleanManagementIssuerStore,
    {
        ...
        let signature =
            genesis_issuance::issue(&candidate, &committee, signature_store, genesis_signer)
                .map_err(|error| match error {
                    genesis_issuance::GenesisIssuanceError::Unavailable => {
                        SharedAgentHostError::Unavailable
                    }
                    genesis_issuance::GenesisIssuanceError::Conflict => {
                        SharedAgentHostError::Conflict
                    }
        ...
        Archive: super::genesis_archive::AgentGenesisArchiveStore,
    {
        ...
        recovery.admission_valid = false;
        let record =
            genesis_issuance::select_and_retain(&candidate, &committee, signatures, archive)
                .map_err(|error| match error {
                    genesis_issuance::GenesisIssuanceError::Unavailable => {
                        SharedAgentHostError::Unavailable
                    }
        ...
        S: CleanManagementReceiptSigner,
    {
        ...
        for (recovery, record) in entries {
            proofs.push(self.verify_published_shared_genesis(
                recovery,
    ...
    pub(crate) fn complete_deferred_shared_genesis(
        &mut self,
        mut proofs: Vec<ReplayVerifiedAgentGenesisFinality>,
    ) -> Result<(), SharedAgentHostError> {
        ...
        for proof in &proofs {
            ...
            if claim.space().0 != target.space.0
                || claim.system_agent().0 != target.system_agent.0
                || claim.authority_binding().0 != target.binding.commitment().0
                || claim.system_genesis() != position.genesis
        ...
        if proofs
            .windows(2)
            .any(|pair| pair[0].0.proposal().locator() == pair[1].0.proposal().locator())
        {
            return Err(SharedAgentHostError::Conflict);
        }
        let agents: Vec<_> = proofs
            .iter()
            .map(|proof| proof.0.proposal().locator().agent)
            .collect();
            ...
            None => {
                ...
                self._network_host.extend_management_pending(
                    ...
                    |(anchor, work)| {
                        ...
                        pending
                            .pledge(publication_store, candidate, committee, record, &target)
                            .map_err(|_| SharedAgentHostError::Unavailable)?;
                        Ok(pending)
                    },
                )?
    ...
    >(
        ...
        let pending = self.prepare_genesis_publication(
            candidate,
            committee,
            record,
            query_store,
        ...
        let decision = genesis_issuance::retain_publication_reply(
            reply_store,
            &pending,
            candidate,
            committee,
        ...
        if !acknowledged {
            ...
            let outcome = self
                ._network_host
                .supervisor_acknowledge_genesis_publication(identity, &proof)?;
        ...
        S: CleanManagementReceiptSigner,
    {
        ...
        if slot
            ...
            .map_err(|_| SharedAgentHostError::Unavailable)?
        {
            self.finish_denied_management_intent(&mut slot, &issuer, local, signer)?;
            return Err(SharedAgentHostError::ScopeMismatch);
        }
        slot.retain_runtime(runtime.exact_bytes())
            .map_err(|_| SharedAgentHostError::Unavailable)?;
        ...
        let created = self.create_local_from_management_intent_with_admission(
            &mut slot,
            managed,
            local,
            runtime,
        ...
        let (agent, acknowledgement) = match created {
            Ok(created) => created,
            Err(error) => {
                #[cfg(test)]
                if self.finalization_failure_once == Some(8) {
                    self.finalization_failure_once = None;
                    return Err(error);
                }
    ...
    pub(crate) fn verify_management_denial<
        B: CleanManagementIssuerStore,
        J: CleanManagementIssuerStore,
    >(
        &mut self,
        ...
        let mut host = self
            ...
            .map_err(|_| SharedAgentHostError::Unavailable)?;
        let Some(input) =
            host.management_denial_invocation_after_anchor(agent, anchor, envelope)?
        else {
            return Ok(None);
        };
        let outcome = host.replay_durable_management_denial(agent, anchor, envelope)?;
        let super::sdk::RuntimeOutcome::Completed(Ok(reply)) = outcome else {
            return Ok(None);
        };
        if reply.invocation != work.invocation
        ...
        if reply.reply
            != crate::actors::codec::Encode::encode(&crate::actors::value::Value::Bytes(Vec::new()))
        {
            return Ok(None);
        }
        Ok(Some(VerifiedManagementDenial {
            call: intent.call().commitment(),
            anchor: anchor.clone(),
            work: envelope.clone(),
            input,
        }))
    ...
    >(
        ...
        let work = slot
            ...
            .ok_or(SharedAgentHostError::ScopeMismatch)?
            .clone();
        let signature = if completed {
            None
        } else {
            if signer.public_key() != target.binding.public_key {
                return Err(SharedAgentHostError::ScopeMismatch);
            }
            Some(
                signer
                    .sign_management_denial_retirement(
                        &slot
                            .denial_signing_bytes()
                            .map_err(|_| SharedAgentHostError::Unavailable)?,
                    )
                    .map_err(|_| SharedAgentHostError::Unavailable)?,
            )
    ...
    mod physical {
        ...
        impl CleanManagementReceiptSigner for CountingSigner {
            ...
            fn sign_management_denial_retirement(
                ...
                self.calls += 1;
                if core::mem::take(&mut self.fail_denial) {
                    return Err(MemoryError);
                }
        ...
        fn inventory_page_table_copies(
                ...
                .enumerate()
            {
                ...
                copies.push(projection_table_copy(
                    ...
                    2 + usize::from(
                        page.next
                            .is_some_and(|cursor| cursor.agent == entry.identity.agent),
                    ),
                ));
                copies.push(projection_table_copy(
                    &case.output,
        ...
        fn inventory_projection_actor(
            ...
            let program = scripted_projection_actor_program(
                vec![credential_routine, first, last, middle],
                descriptor_table,
            );
        ...
        #[test]
        fn native_local_lifecycle_denial_is_proven_by_fresh_replay_without_issuance() {
            native_local_management_lifecycle(26);
        }

        #[test]
        fn native_local_lifecycle_unissued_approval_is_not_a_denial() {
            native_local_management_lifecycle(27);
        }

        #[test]
        fn native_local_lifecycle_denial_retires_and_unblocks_valid_creation() {
            native_local_management_lifecycle(28);
        }

        #[test]
        fn native_local_lifecycle_denial_retries_signing_after_positive_ack() {
            native_local_management_lifecycle(29);
        }

        #[test]
        fn native_local_lifecycle_denial_releases_after_ambiguous_record_commit() {
            native_local_management_lifecycle(30);
        }

        #[test]
        fn native_local_lifecycle_denial_startup_recovers_after_positive_ack() {
            native_local_management_lifecycle(31);
        }
        ...
        #[inline(never)]
        fn check_management_denial_evidence(
            harness: &mut NativeProjectionOwnerHarness,
            descriptor: AgentDescriptor,
            ...
            let denied = scenario != 27;
            if denied {
                // A genuine signed request with an unavailable next sequence:
                // policy returns canonical empty bytes, not a runtime failure.
            ...
            let mut local = crate::agent::local_sdk_host::LocalAgentHost::create(
                &harness._directory.0.join("denial-local"),
                descriptor.identity.space,
                owner.pins.node,
                harness.fixture.trust.clone(),
            )
            .unwrap();
            let mut intent_store = IssuerMemoryStore::default();
            let issuer_store = IssuerMemoryStore::default();
            let mut signer = CountingSigner::new();
            if scenario == 30 {
                intent_store.fail_retirement_after_commit =
                    Some(Arc::new(std::sync::atomic::AtomicBool::new(true)));
            }
            signer.fail_denial = matches!(scenario, 29 | 31);
            let before = owner.ordered_index_for_test().unwrap();
            if scenario < 28 {
                owner.fail_finalization_once_for_test(if denied { 8 } else { 6 });
            }
            assert_eq!(
            ...
            if scenario >= 28 {
                ...
                if scenario == 31 {
                    ...
                    fn restart_denial(
                        ...
                        crate::agent::local_sdk_host::LocalAgentHost,
                        CountingSigner,
                    ) {
                        struct Stores {
                        ...
                        impl crate::agent::local_lifecycle::LocalLifecycleStoreFactory for Stores {
                            ...
                            fn discover(
                                ...
                                if space != self.scope.0 || maximum < 1 {
                                    return Err(());
                                }
                                Ok(vec![self.scope.1])
                            }
                        ...
                        let before = old.ordered_index_for_test().unwrap();
                        let pins = old._pins_store.clone();
                        let record = old.record_store.clone();
                        let bootstrap_issuer = old.issuer.into_store();
                        ...
                        let recovery =
                            ...
                            .unwrap();
                        let admission = recovery.startup_admission().unwrap();
                        assert_eq!(admission.pending.len(), pending);
                        let owner = CleanSystemAgentBootstrapOwner::open_or_bootstrap_with_factory(
                            pins,
                            record,
                            bootstrap_issuer,
                            &mut signer,
                            || panic!("denial recovery cannot bootstrap"),
                            harness._directory.host(),
                            harness._directory.lock(),
                            harness.fixture.plan.pins.space,
                            harness.fixture.plan.pins.node,
                        ...
                        let controller =
                            crate::agent::local_lifecycle::LocalLifecycleController::with_recovery(
                                owner, local, stores, signer, recovery,
                            )
                            .unwrap();
                    ...
                    for _ in 0..2 {
                        (owner, local, signer) = std::thread::scope(|scope| {
                            scope
                                .spawn(|| {
                                    restart_denial(
                                        owner,
                ...
                let signatures = if matches!(scenario, 29 | 31) { 2 } else { 1 };
                ...
                *corrupt_store.image.lock().unwrap() = Some(corrupt);
                ...
                next.creation_nonce = Hash([0xb9; 32]);
                next.identity.agent = AgentId::derive(
                    next.identity.space,
                    next.identity.owner,
                    next.creation_nonce.as_bytes(),
                );
                let key = SigningKey::from_bytes(&[CREDENTIAL_SEED; 32]);
                let (mut next_call, _) = credential_call_and_approval(
                    &next,
                    &ManagementRequest::Create(Box::new(next.clone())),
                    &key,
                );
                ...
                let result = owner
                    ...
                    .unwrap();
                assert_eq!(result.0, next.identity.agent);
                result.1.verify_with(&RawCredentialVerifier).unwrap();
            ...
            for attempt in 0..2 {
                ...
                let proof = owner
                    .verify_management_denial(&slot, &issuer, &local)
                    .unwrap();
                assert_eq!(proof.is_some(), denied);
                if let Some(proof) = proof {
                    assert_eq!(proof.call, call.commitment());
                    assert_eq!(&proof.anchor, slot.authorization_anchor().unwrap().unwrap());
                    assert_eq!(&proof.work, slot.authorization_work().unwrap().unwrap());
                    assert_eq!(
                        Some(proof.input),
                        owner
                            .host
                            .lock()
                            .unwrap()
                            .management_invocation_after_anchor(agent, &proof.anchor, &proof.work)
                            .unwrap()
                    );
                }
                assert_eq!(owner.ordered_index_for_test().unwrap(), before + 1);
                ...
                assert_eq!(signer.calls, 0);
                // Evidence collection must not release or retire anything.
                owner
                    ._network_host
            ...
            let wrong_issuer = DurableCleanManagementIssuer::open(
                IssuerMemoryStore::default(),
                descriptor.authority,
                descriptor.identity.space,
                AgentId([0x59; 32]),
            ...
            .unwrap();
            assert!(matches!(
                owner.verify_management_denial(&slot, &wrong_issuer, &local),
                Err(SharedAgentHostError::ScopeMismatch)
            ));
            assert_eq!(owner.ordered_index_for_test().unwrap(), before + 1);
        }
        ...
        fn native_bundled_authority_fixture_with_query_catalog(query: bool) -> PhysicalFixture {
            native_authority_package_fixture(
                query,
                include_bytes!("../../../vosx/blobs/system_authority.vos"),
            )
        }
        ...
        fn native_local_management_lifecycle(coordinated: u8) {
            ...
            if matches!(coordinated, 26 | 27 | 28 | 29 | 30 | 31) {
                std::thread::scope(|scope| {
                    scope
                        .spawn(|| {
                            check_management_denial_evidence(
                                &mut harness,
                                descriptor,
                                call,
                                runtime,
            ...
            if matches!(coordinated, 20 | 21 | 22 | 23 | 24 | 25) {
                // Keep native setup frames off the default-sized lifecycle
                // execution stack, just as for the denial fixture above.
                std::thread::scope(|scope| {
                    scope
                        .spawn(|| {
                            check_dependent_lifecycle_recovery(
                                &mut harness,
                                descriptor,
                                runtime,
                                coordinated,
        ...
        fn check_multiple_management_retirement_admission(
            ...
            **authorization = InvocationAuthorization::PublicPreflight(
            ...
            owner
                ...
                .retire_attachment_for_test(agent)
                .unwrap();
            assert!(matches!(crate::network::shared_agent::SharedAgentNetworkHost::attach_recovering_management_set(
                Arc::clone(&owner.host), Arc::clone(&network), agent,
                vec![(management_anchor_for_test(&first_anchor), extra[0].clone())],
            ...
            **authorization = InvocationAuthorization::PublicPreflight(
            ...
            assert!(matches!(
                owner
                    ._network_host
                    .complete_management_retirement(agent, original, || Err(
                        SharedAgentHostError::Unavailable
                    )),
                Err(SharedAgentHostError::Unavailable)
            ));
            ...
            owner
                ._network_host
                .handoff_management_pending_to_retirement(agent, &[[&extra[2], &extra[3]]])
                .unwrap();
            owner
                ._network_host
                .reserve_management_retirement_set(
                    agent,
                    &[[&extra[0], &extra[1]], [&extra[2], &extra[3]]],
                )
                .unwrap();
        ...
        fn check_local_management_phases(
            ...
            check_management_retirement_gate(owner, [&envelope, &finalization_envelope], true);
            {
                ...
                let agent = HostAgentId(owner.pins.agent.0);
                assert!(
                    host.management_denial_invocation_after_anchor(
                        agent,
                        &authorization_anchor,
                        &envelope
                    )
                ...
                // The denial walker can identify the old Invoke, but an
                // approved result must never acquire denial-only admission.
                assert!(
                    host.management_pending_admission_requirement(
                        agent,
                        &[(&authorization_anchor, &envelope)]
                    )
                    .is_err()
                );
            }
        ...
        fn check_genesis_signature_crash_boundaries(
            ...
            for (fail_at, after_write) in [(1, false), (1, true), (2, false), (2, true)] {
                let mut signer = Signer {
                    key: SigningKey::from_bytes(&[0xe7; 32]),
                    messages: Vec::new(),
                    invalid: false,
                };
                let committee = AuthorityCommittee::new(
                    candidate.claim().space(),
                    candidate.claim().authority_binding(),
                    1,
                    None,
                    vec![
                        AuthorityCommitteeMember::new(
                            crate::service::NodeId([0xe8; 32]),
                            signer.public_key(),
                            AuthorityMemberRole::Voter,
                        )
                        .unwrap(),
                ...
                assert_eq!(
                    issue(candidate, &committee, &mut store, &mut signer),
                ...
                assert!(
                    signer
                        .messages
                        .iter()
                        .all(|message| message == &signer.messages[0])
                );
                assert_eq!(
                    issue(candidate, &committee, &mut reopened, &mut signer).unwrap(),
                    signature
                );
                ...
                .unwrap();
                certificate
                    .verify(&committee, candidate.claim().authority_claim())
                    .unwrap();
                let mut changed = candidate.clone();
                changed.authorization = InvocationId([0xef; 32]);
                assert_eq!(
                    issue(&changed, &committee, &mut reopened, &mut signer),
                ...
                reopened
                    ...
                    .map(|byte| *byte ^= 1);
                assert_eq!(
                    issue(candidate, &committee, &mut reopened, &mut signer),
                ...
                signer.invalid = true;
                assert_eq!(
                    issue(candidate, &committee, &mut fresh, &mut signer),
                    Err(GenesisIssuanceError::InvalidSignature)
                );
            ...
            let mut signers: Vec<_> = [0xe1, 0xe2, 0xe3]
                .into_iter()
                .map(|seed| Signer {
                ...
                .collect();
            let mut members: Vec<_> = signers
                .iter()
                .enumerate()
                .map(|(index, signer)| {
                    AuthorityCommitteeMember::new(
                        crate::service::NodeId([index as u8 + 1; 32]),
                        signer.public_key(),
                        AuthorityMemberRole::Voter,
                    )
                    .unwrap()
            ...
            let signatures: Vec<_> = signers
                .iter_mut()
                .map(|signer| issue(candidate, &committee, &mut Store::default(), signer).unwrap())
                .collect();
            ...
            for after_write in [false, true] {
                ...
                let selected = super::super::genesis_issuance::select_and_retain(
                    candidate,
                    &committee,
                    second.clone(),
                    &archive,
                ...
                assert_eq!(
                    ...
                    if after_write {
                        ...
                        assemble(candidate, &committee, second).unwrap()
                    }
                );
                ...
                let publication = super::super::genesis_issuance::publication_input(
                    candidate, &committee, &selected,
                )
                .unwrap();
                assert_eq!(
                ...
                assert!(
                    publication
                        .provision
                        .reference
                        .matches(&publication.provision.bytes)
                );
                assert_eq!(
                    publication.provision.bytes,
                    crate::service::ServiceWire::encode(selected.provision())
                );
                assert_eq!(publication.message[0], crate::actors::value::TAG_DYNAMIC);
                let message: crate::actors::value::Msg =
                    crate::actors::codec::Decode::decode(&publication.message[1..]);
                assert_eq!(
                    message.encode(),
                    crate::actors::value::Msg::new("publish_genesis")
                        .with(
                            "authorization",
                            crate::actors::value::Value::Bytes(
                                candidate.authorization().0.to_vec()
                            )
                        )
                        .with(
                            "provision_hash",
                            crate::actors::value::Value::Bytes(
                                publication.provision.reference.hash.0.to_vec()
                            )
                        )
                        .with(
                            "provision_len",
                            crate::actors::value::Value::U64(publication.provision.reference.len)
                        )
                        .encode()
                );
                let mut substituted = candidate.clone();
                substituted.claim = crate::agent::genesis::AgentGenesisClaim::new(
                    candidate.claim().system_agent(),
                    crate::agent::journal::AgentJournalGenesisId::new([0x97; 32]),
                    candidate.claim().system_admission(),
                    candidate.proposal(),
                    candidate.replicas(),
                )
                .unwrap();
                assert!(
                    super::super::genesis_issuance::publication_input(
                        &substituted,
                        &committee,
                        &selected
                    )
            ...
            .unwrap();
            assert_eq!(
                crate::agent::genesis::AgentGenesisProvider::reproduce(
                    &archive,
            ...
            let next = AuthorityCommittee::new(
                candidate.claim().space(),
                candidate.claim().authority_binding(),
                2,
            ...
            signature[0] ^= 1;
            forged[0] =
                crate::agent::committee::AuthoritySignature::new(forged[0].signer(), signature)
                    .unwrap();
            ...
            let foreign = crate::agent::committee::AuthoritySignature::new(
                crate::agent::committee::AuthoritySignerId::of_raw_ed25519(
            ...
            assert!(
                crate::agent::genesis::VerifiedAgentGenesisProvision::verify(
                    record.provision().clone(),
                    &NoFinality,
                )
                .is_err()
        ...
        fn native_shared_committee_query_preparation_uses_candidate_package() {
            let bytes = std::fs::read(
                std::env::var("AUTHORITY_CANDIDATE_PACKAGE")
                    .expect("set AUTHORITY_CANDIDATE_PACKAGE to the fresh signed .vos package"),
            )
            .unwrap();
            check_shared_proposal_and_committee_preparation(
                native_authority_package_fixture(false, &bytes),
                true,
            );
        }
        ...
        fn check_shared_proposal_and_committee_preparation(
            ...
            prepared
                .claim()
                .validate_against(prepared.proposal(), prepared.replicas())
                .unwrap();
            ...
            .unwrap();
            assert_ne!(
                foreign_claim.authority_claim(),
                prepared.claim().authority_claim()
            );
            check_genesis_signature_crash_boundaries(
                &prepared,
                slot.authorization_work().unwrap().unwrap(),
                owner.authority_target(),
                anchor,
            ...
            assert!(
                owner
                    ...
                    .lock()
                    .unwrap()
                    .show(HostAgentId(descriptor.identity.agent.0))
                    .unwrap()
                    .is_none()
            );
            ...
            .unwrap();
            assert_eq!(
                leased_recovery.runtime().unwrap().exact_bytes(),
                runtime.exact_bytes()
            );
            ...
            for image in [None, Some(vec![1, 2, 3])] {
                let missing_or_corrupt = IssuerMemoryStore {
                    runtime: Arc::new(Mutex::new(image)),
                    ..intent_store.clone()
                };
                assert!(
                    super::super::NativeSharedGenesisRecovery::open(
            ...
            let admission =
                super::super::NativeAuthorityOperationStartupAdmission::from_shared_genesis(
                    &mut leased_recovery,
                )
                .unwrap();
            ...
            assert_eq!(
                ...
                owner.authority_target().binding.commitment().0
            );
            assert_eq!(queried.members().len(), 1);
            assert_eq!(
                queried.members()[0].public_key(),
                &SigningKey::from_bytes(&[CREDENTIAL_SEED; 32])
                    .verifying_key()
                    .to_bytes()
            );
            ...
            let (resumed, resumed_committee) = owner
                .resume_shared_genesis_preparation(&mut leased_recovery, &committee, &mut signer)
                .unwrap();
            ...
            let mut foreign = GenesisSigner {
                key: SigningKey::from_bytes(&[0x9e; 32]),
                calls: 0,
            };
            assert!(
                owner
            ...
            let mut genesis_signer = GenesisSigner {
                key: SigningKey::from_bytes(&[CREDENTIAL_SEED; 32]),
                calls: 0,
            };
            let endorsed = owner
                .endorse_recovered_shared_genesis(
            ...
            let repeated_endorsement = owner
                .endorse_recovered_shared_genesis(
                    &mut leased_recovery,
                    &committee,
                    &mut signer,
                    &mut signature_store.clone(),
                    &mut genesis_signer,
                )
                .unwrap();
            ...
            assert_eq!(owner.ordered_index_for_test().unwrap(), after_query);
            let record = super::super::genesis_issuance::assemble(
                &endorsed.0,
                &endorsed.1,
                vec![endorsed.2],
            )
            ...
            publish_work
                .availability
                .push(publication.provision.clone());
            publish_work
                .availability
                .sort_unstable_by(|a, b| a.reference.cmp(&b.reference));
            let mut material = owner
            ...
            let accepts = |work: &crate::agent_sdk::InvocationWork| {
                ...
                crate::agent::supervisor_adapters::physical_material_authorizes_work(
                    &material,
                    identity,
                    RuntimeExecutionContext::Direct,
                    work,
            ...
            missing
                .availability
                .retain(|blob| blob.reference != publication.provision.reference);
            assert!(!accepts(&missing));
            let mut changed = publish_work.clone();
            changed
                .availability
                .iter_mut()
                .find(|blob| blob.reference == publication.provision.reference)
                .unwrap()
                .bytes
                .push(0);
            assert!(!accepts(&changed));
            let mut changed = publish_work.clone();
            changed.availability.push(crate::agent_sdk::RuntimeBlob {
            ...
            changed
                .availability
                .retain(|blob| blob.reference != material.schema.reference);
            assert!(!accepts(&changed));
            let mut publication_store = IssuerMemoryStore::default();
            let retained_publication = owner
            ...
            use super::super::genesis_issuance::{
                GenesisIssuanceError, RetainedGenesisPublication,
            };
            assert!(
            ...
            for after_write in [false, true] {
                ...
                if after_write {
                    ...
                    assert_eq!(
                        RetainedGenesisPublication::load(
                            &mut failed,
                            &prepared,
                            &queried,
                            &record,
            ...
            assert_eq!(
                conflicting.load().unwrap().as_deref(),
                Some(b"different publication".as_slice())
            );
            ...
            assert_eq!(owner.ordered_index_for_test().unwrap(), after_query);
            let RuntimeWork::Invoke {
                invocation: published_work,
                authorization: published_auth,
                ..
            ...
            let mut before_dispatch = super::super::NativeSharedGenesisRecovery::open(
                ...
                missing_reply.clone(),
                publication_store.clone(),
                untouched_reply.clone(),
            )
            .unwrap();
            assert!(
                owner
                    .verify_published_shared_genesis(
                        &mut before_dispatch,
                        &committee,
                        &mut signer,
                        &record
            ...
            for after_write in [false, true] {
                ...
                assert_eq!(
                    owner.execute_genesis_publication(
                        &prepared,
                        &queried,
                        &record,
                        &mut reopened_query,
            ...
            let finality = owner
                .verify_published_shared_genesis(&mut publishing, &committee, &mut signer, &record)
                .unwrap();
            assert!(
                crate::agent::genesis::VerifiedAgentGenesisProvision::verify(
                    record.provision().clone(),
            ...
            let alternate_committee = crate::agent::committee::AuthorityCommittee::new(
                queried.space(),
                queried.authority_binding(),
                queried.epoch() + 1,
                Some(queried.commitment()),
                queried.members().to_vec(),
            )
            .unwrap();
            let alternate_signature = super::super::genesis_issuance::issue(
                &prepared,
                &alternate_committee,
            ...
            .unwrap();
            let alternate_record = super::super::genesis_issuance::assemble(
                &prepared,
                &alternate_committee,
                vec![alternate_signature],
            )
            .unwrap();
            assert_eq!(
            ...
            assert_eq!(
                super::super::ReplayVerifiedAgentGenesisFinalitySet(Vec::new())
            ...
            assert_eq!(
                owner.complete_deferred_shared_genesis(vec![finality]),
                Err(SharedAgentHostError::ScopeMismatch)
            );
            assert_eq!(
                owner.complete_deferred_shared_genesis(Vec::new()),
            ...
            assert_eq!(
                super::super::genesis_issuance::load_publication_reply(
                    &mut missing,
                    &retained_publication
                )
                .unwrap(),
            ...
            assert_eq!(
                owner
                    .resume_shared_genesis_preparation(&mut recovered, &committee, &mut signer)
                    .unwrap(),
            ...
            assert_eq!(
                super::super::genesis_issuance::load_publication_reply(&mut missing, &wrong_work),
                Err(GenesisIssuanceError::Conflict)
            );
            for after_write in [false, true] {
                let mut failed = ReplyStoreFailure {
