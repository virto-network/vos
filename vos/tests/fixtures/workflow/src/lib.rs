...
pub struct Workflow {
    value: u32,
    #[storage(prefix = "workflow/counters/")]
...
impl Workflow {
    ...
    async fn spawn_child(&mut self, ctx: &mut Context<Self>, name: String, initial: u32) -> bool {
        ...
        ctx.spawn::<WorkflowRef>(name, &child).await.is_ok()
    }
    ...
    async fn call_child(&mut self, ctx: &mut Context<Self>) -> u32 {
        self.value += 10;
        if let Ok(mut child) = ctx.child::<WorkflowRef>("child").await
    ...
    #[msg]
    fn ipc_tail(&self) -> u8 {
        let address = vos::service::ACTOR_IPC_BASE_PAGE as usize * 4096 + 1024;
            ...
            .await
        {
            Ok(package) => {
                package.value == Value::U32(7)
                    ...
                    && <vos::service::AttestationProofManifest as vos::service::ServiceWire>::decode(
                        &package.proof,
                    )
                    .is_ok()
            }
