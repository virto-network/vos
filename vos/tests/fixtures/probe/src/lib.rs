...
struct Probe {
    seen: u32,
    /// Point-read fixture for the service transport path. The default is
...
impl Probe {
    ...
    /// accumulated peer reply after restart. Runtime service integration tests use
    ...
    /// Deterministic cross-root peer used by the service durable transport gate.
    #[msg]
    ...
    /// The service physical gate restarts the service before each continuation
    ...
    async fn await_two_peers(&mut self, ctx: &mut Context<Self>) -> u32 {
        ...
        for key in 0..vos::service::MAX_ACTOR_STORAGE_WITNESSES as u64 {
    ...
    /// producer record to bind the exact Task reply before returning. The service
