...
//!
//! Cuts the WHOLE messenger crate (`extensions/messenger`) to a no_std
...
//!
//! The messenger stays a host-workspace member (so `cargo test -p
//! messenger-extension --lib` still drives the .so flavor), so its ELF lands in
//! the shared workspace target dir, not a crate-local one. Build it with
//! `cd extensions/messenger && cargo +nightly actor`. If the ELF is absent the
