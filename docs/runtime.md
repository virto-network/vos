...
> APIs, and CRDT primitives described here are present. The local service harness
...
> explicitly opened Local, Raft, or CRDT service root and route ordinary public
...
> installed Raft `space-authority`; caller-supplied legacy role bytes are ignored. Signed service
...
> not evidence of service conformance.
...
`LocalRootTreeService` is the reusable single-writer ownership boundary for
...
`VosNode::register_service_root_at_id` attaches that boundary without extracting
the actor PVM into the legacy runtime. Its strict `RootTreeInvocation` keeps
...
both check the full `ServiceIdentity` committed in the message; a node route
...
manifest remains below the existing 64-KiB service proof-object limit, while each
...
`CrdtSyncEnvelope` carries advertised heads, canonical causal nodes, the
...
`ReceiptVerificationRequest` sidecar locally before entering IC-5. Missing,
...
with either role annotation is never installed as public. The service actor dispatch
...
decoder and was remotely reachable, so it is not part of the service ABI.

This service cutover changes two application reply wires. `Option<T>` uses
...
with the repinned service service artifact.
...
When service genesis pins a `RoleAuthorityBinding`, disclosed space-role
credentials instead carry an `AccumulatedRoleAssertion`. Guest Accumulate
...
`ReceiptVerificationRequest` beside `AdmitIngress` in the committed entry;
...
The current legacy `vosx space publish` path does not activate this service Install
entry. Production service installation must resolve a signature-verified `.vos`
package, derive the exact `ActorGenesis` (including its canonical role-policy
...
receipt-bound `AccumulatedTimeout`. Linear mode advances the revision; CRDT
...
`--allow-conformance` seam. The two flags are mutually exclusive, and the
...
credential, and receipt. Structured payloads are their canonical `ServiceWire`
...
one service root is opened synchronously before endpoint publication; on a
...
`just test-production-daemon` is the physical cutover gate. It builds the
...
`just package-production-release` reproduces the ABI-17 authority PVM, then
...
`register_service_raft_root_at_id_production*` entry points install the production
...
Refine is pure. A `WorkEnvelope` imports the exact deployment, program,
state, continuation pages, authorization evidence, causal base and referenced
blobs needed to run a slice. Refine may only return a `Transition`; it cannot
...
operation that creates an owned child. In this service slice it creates a
...
`Apply` request carries the `AccumulationEnvelope::provided_blobs` needed by
...
`RaftAccumulateLog` is the redb/`vos-raft` implementation of that boundary.
...
descriptor. The service identity remains the genesis identity even after the
catalog names the new actor deployment.
...
Raft roots without a sealed production trust policy fail closed because a
leader's process-local conformance allowlist cannot replay on followers. Roots
whose current or replacement package exposes attested methods, or whose upgrade
would add or remove the root's role-authority requirement, also fail closed.
Existing attested handles and authority bindings are immutable external trust
inputs; updating either needs its own guest-owned migration before an in-place
target upgrade can be safe.

## Packages and identity

`.vos` service packages bind the service ABI, execution-semantics ID, canonical
...
authenticate its witness layout. Service installs actor INVOKE only for
...
blocking-work limits plus a bounded root queue prevent identity rotation or a
...
`--allow-conformance`. It opens each ordinary Local or Raft deployment as
...
probe, or join action, so a refused service row cannot change quorum membership.
...
A joining service Raft root publishes fail-closed pending policy metadata before it
...
Service CRDT roots admit anti-entropy only from a full Noise identity present in the
...
canonical `ReceiptVerificationRequest`; `Deliver` carries the source receipt
...
Registry-level pointer mutation alone remains invalid for signed service packages.
The `space upgrade` command recognizes a to change and first runs the
guest-owned `UpgradeActor` protocol described above, then performs the catalog
compare-and-swap. Crossing the legacy/service boundary and CRDT upgrades fail
...
recorded in `support/production-artifacts.toml`. To reproduce it from that
immutable provenance and validate the guest:

```sh
just build-vos-service
cmp target/pinned-production-artifacts/vos-service.pvm \
...
output, and requires byte identity with the committed PVM. The service service
...
reinstalled; there is no v1 decoder or migration in a service service.
...
are therefore a clean break from earlier experimental service images. ABI 11 bound
...
and accepts the irreversible call only when its authenticated causal source
matches both the pinned service identity and ActorId. Actor IDs may be reused
across roots, so actor identity without a source service fails closed; another
...
in clerk-ledger; both direct and provable transfer paths reject every later
`void_of` targeting the locked transfer. Thus a void racing issuance either
commits before the anchor and prevents issuance, or loses to the lock and is
rejected before the bridge resumes. The bridge compares the currency before it
signs and accumulates the issuer commitment in one accepted slice.
Receiver-side `redeem_voucher` enforces the same currency/layer and
no-pending/no-void policy on its inflow. Exact retries return the stored voucher
without signing or accumulating twice.
...
corresponding shared lock and also refuses a live legacy endpoint. Restore and
`space forget` take the exclusive lock before replacing or deleting bytes. The
daemon must therefore be stopped before any bytes are copied or removed. The
lock file lives outside the mutable data directory, so restore cannot evade it
by renaming the directory being protected.
...
metadata/logs, CRDT DAGs, service applied images, proof artifacts, producer-private
...
does not equal their content digest fail closed. The archive and its `data/`
and `blobs/` roots must be real directories, and the manifest must contain a
non-empty `node.key`, registry database, and exact declared registry blob.
Backup is assembled under a temporary sibling, verified in full, and only then
renamed to its requested path. A space whose one-shot genesis recipe is still
pending is refused because that external recipe is not durable space state;
boot it once before backup. The printed manifest digest can be retained out of
band when the storage provider is not trusted; the in-archive manifest by
itself establishes self-consistency and corruption detection, not an external
signature.
...
publication. `just test-release-operations` physically commits a signed
...
Every actor slice records its complete `WorkEnvelope`; synchronized peers can
...
commits the complete `WorkEnvelope` hash. An exact retry therefore reuses the
