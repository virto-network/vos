# Production service release operations

This runbook covers the artifacts and offline state that an operator needs to
move a production service node without changing protocol identity. It is deliberately
...
`support/production-artifacts.toml`, runs that revision's transpiler, checks
the fresh PVM digest and ProgramId, and requires it to match the committed PVM
byte-for-byte. The fresh output is
`target/pinned-production-artifacts/vos-service.pvm`. A clone must therefore retain or
...
just package-production-release target/production-release
...
- `manifest.json`, which binds both file sizes, raw BLAKE2b-256 digests,
  program identities, the service ABI, store schema, and execution-semantics ID.
...
Before publishing the directory, run `just test-release-operations`. The
...
A service root whose local policy sets `device_secret = true` receives a
host-private CipherClerk signer through the Refine-only `DEVICE_SIGN`
capability. The daemon stores the 32-byte seed at
`services/<root-service-id>.device-seed` with mode `0600`; actor memory,
...
service Local and Raft roots use `vosx space upgrade`, which drives the guest-owned
