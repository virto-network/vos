//! Recursion build P1: a **Poseidon2-over-M31 Fiat-Shamir channel**, so the
//! transcript is M31-algebraic end-to-end — NO Blake2s anywhere on the
//! commit/transcript path. Combined with the de-risked Poseidon2-M31 Merkle
//! commitment (`poseidon2_pcs_spike.rs`), this gives a fully M31-algebraic PCS:
//! the substrate native recursion needs (the verifier-AIR must replay this
//! transcript in-circuit as ~397 Poseidon2 permutations, which is only tractable
//! when the transcript hash is the same algebraic permutation as the Merkle
//! hash).
//!
//! ## Gate
//!
...
//! changes subsequent draws). Constants are the placeholder `1234`; vetted
//! width-16 M31 round constants (+ a published-vector known-answer check) are a
//! documented follow-up swap — the transcript PLUMBING is independent of constant
//! quality (prover and verifier run the same channel, so they agree by
//! construction).
//!
...
// ── Poseidon2-over-M31 permutation (width 16) — shared with the spike/P2 ──
...
// ── Poseidon2-M31 Merkle hasher (the de-risked PCS commitment) ────────────
...
// ── Poseidon2-M31 Fiat-Shamir channel (the P1 deliverable) ────────────────
