...
/// derivation — together with the BitwiseAnd / Range256 lookups those
/// witnesses require, plus the GateH / InitGateH / OutputGateH helper
/// definitions.  Shared verbatim by `Blake2bChip` (which adds the
...
>(
    ...
    // method so output-derivation relation entries are included in the pairing.

    // ── Carry / rot-carry bounds (helper-flattened) ──
    ...
    for i in 0..8 {
        ...
        eval.add_constraint(carry1_xcm1[i].clone() - c1_v.clone() * (c1_v.clone() - f1.clone()));
        eval.add_constraint(carry1_full[i].clone() - carry1_xcm1[i].clone() * (c1_v - f2.clone()));
        eval.add_constraint(is_real[0].clone() * carry1_full[i].clone());

        // Carry3: same.
        let c3_v = carry3[i].clone();
        eval.add_constraint(carry3_xcm1[i].clone() - c3_v.clone() * (c3_v.clone() - f1.clone()));
        eval.add_constraint(carry3_full[i].clone() - carry3_xcm1[i].clone() * (c3_v - f2.clone()));
    ...
    // V_next update (helper-flattened): for slot k
    ...
    for k in 0..16 {
        for i in 0..8 {
            eval.add_constraint(gate.clone() * (m_cols_next[k][i].clone() - m_cols[k][i].clone()));
        }
    }
    ...
    // Gate substitution: init_gate ← InitGateH.
    ...
    for i in 0..8 {
        ...
        // V[0..8] = H_k
        for k in 0..8 {
            eval.add_constraint(init_gate.clone() * (v_cols[k][i].clone() - h_cols[k][i].clone()));
        }
        // V[8..12] = IV[0..4]
    ...
    // ── Output derivation at row 95 of each compression ──
...
impl BuiltInComponent for Blake2bChip {
    ...
    fn add_constraints<E: EvalAtRow>(
        ...
        // Gate substitution: init_gate_8b ← InitGateH (alias).
        ...
        // compression (output-derivation witness); HWrAddr/CallTs stay constant.
        // Gate substitution: write_gate_8b ← OutputGateH (alias).
        ...
        // ── Consumer linking to CpuChip ECALL step ──
...
pub(in crate::chips::blake2b) fn build_compression_rows(
    ...
    for (call_idx, call) in calls.iter().enumerate() {
        ...
        // ECALL-binding data for this compression, if a matching
        ...
        for round in 0..12 {
            ...
            for g_idx in 0..8 {
                ...
                // Final G-call of this compression → populate
                // output witnesses from the just-updated v.
                if round == 11 && g_idx == 7 {
                    fill_output_witnesses(&mut row, &v);
                }

                // ECALL-binding fields, constant across 96 rows.
                row.h_ptr = h_ptr;
                row.m_ptr = m_ptr;
                row.call_ts = call_ts;
...
impl BuiltInProverComponent for Blake2bChip {
    ...
    fn generate_interaction_trace(
        ...
        // Re-read the columns the ECALL-binding lookups consume from the core region.
        ...
        // ── Per-byte memory-access consumer lookups ──
        ...
        // (output-derivation witness).  HWrAddr/CallTs are inter-row-constant so they
        ...
        // ── Blake2b-call consumer linking to CpuChip ECALL step ──
