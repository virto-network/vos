...
//!
//! `RistrettoChip` witnesses one p25519 field operation (add / sub /
//! mul) per row and pins its byte-decomposition via the shared
//! `field_op_constraints` algebra, with Range256 range-checks on every
//! committed byte and a register-file lookup binding each row's inputs
//! to a prior row's output.  `active_components` selects the chip based
//! on `ChipActivity.ristretto`, which `activity_from_steps` flips on
//! when the trace contains an `ECALL_RISTRETTO_SCALAR_MULT` step.
//! Padding rows carry `is_real = 0` and contribute nothing.
...
/// Derive the chip's log_size from `side_note
...
///
/// One row witnesses ONE p25519 field operation (add, sub, or mul;
/// inv composes mul rows).  Edwards point ops compose multiple
/// field-op rows; the scalar-mult main loop schedules rows +
/// boundary lookup cells around them.
///
...
pub enum Column {
    ...
    /// Each entry is 0 or 1.  Pinned via the per-byte sum
    /// constraint chain.
    #[size = 32]
    AddCarry,
    /// 1 iff the unreduced sum was ≥ p (so out = intermediate − p);
    /// 0 if out = intermediate directly.  Determinism (output < p) is
    /// additionally pinned via a finalize chain.
    #[size = 1]
    IsOverflow,
    ...
    /// 64-byte unreduced schoolbook product `a · b` for is_mul
    ...
    /// Reduction `mod p` happens separately in the reduction passes
    /// (this is the pre-reduction product).
    #[size = 64]
    MulProduct,
    /// Per-position low byte of the schoolbook carry chain.
    ...
    /// the per-position schoolbook constraint chain.
    #[size = 64]
    MulCarry,
    /// Per-position middle byte of the schoolbook carry.
    #[size = 64]
    MulCarryMid,
    /// Per-position high (upper) byte of the schoolbook carry.
    #[size = 64]
    MulCarryHi,

    /// Pass-1 reduction fold `lo + 38·hi` (mod p), where
    ...
    #[size = 32]
    Pass1Lo,
    /// Pass-1 overflow into bytes 32..34 (≤ 38, so 2 bytes
    /// is plenty).
    #[size = 2]
    Pass1Hi,
    /// Per-position carry-byte chain (lo + mid bytes per
    ...
    /// Pass-2 fold `pass1_lo + 38·pass1_hi`.  Since
    ...
    #[size = 1]
    Pass2CarryOut,
    /// Per-position carry chain for the pass-2 fold.
    #[size = 32]
    Pass2Carry,

    /// Top bit of pass2_lo[31] (= bit 255 of the value).
    /// If set, the value is in `[2²⁵⁵, 2²⁵⁶)` and reduces by adding
    /// 19 after clearing bit 255.
    #[size = 1]
    Pass2TopBit,
    /// Pass2 with bit 255 cleared and +19 added if either
    ...
    #[size = 32]
    AfterTopBit,
    /// Per-position carry chain for the +19 step.
    #[size = 32]
    AfterTopCarry,

    /// Operation classifier flags — exactly one is 1 on a real row.
    /// is_inv composes mul rows + a Fermat ladder driver, with no
    /// separate row class.
    #[size = 1]
    IsAdd,
    #[size = 1]
    IsSub,
    /// 1 iff this row witnesses a 256-bit × 256-bit field
    /// multiplication.  Mutually exclusive with IsAdd / IsSub on
    /// real rows.
    #[size = 1]
    IsMul,
    /// 1 iff this row is a BOUNDARY-INPUT producer row.
    ...
    #[size = 1]
    IsInput,
    /// 1 iff this row is a BOUNDARY-OUTPUT consumer row.
    ...
    /// balances.  The ECALL OUTPUT boundary takes this role via
    /// MemoryChip's write entries; for chip-only tests, this row
    /// class lets us close the chain.
    #[size = 1]
    IsOutput,

    /// Row-id (low byte) of the row that produced this
    ...
    #[size = 1]
    ASourceRowHi,
    /// Row-id of the row that produced this row's `b` input.
    #[size = 1]
    BSourceRowLo,
    #[size = 1]
    BSourceRowHi,
...
pub enum PreprocessedColumn {
    /// Reserved placeholder column.
    #[size = 1]
    Reserved,
    /// This row's index, low byte (rows 0..256).
    /// Used as the "row_id_lo" element of producer lookup tuples
    /// emitted on RistrettoRegisterFileLookupElements.
    #[size = 1]
    RowIndexLo,
    /// This row's index, high byte (256·byte for
    /// chip log_size up to 16).
    #[size = 1]
    RowIndexHi,
    /// Byte-index constants {0, 1, ..., 31} packed into
...
impl BuiltInComponent for RistrettoChip {
    /// Every schoolbook position uses MulPartialSum[k] (deg-1 helper)
    /// gated by RealMulH (deg-1 helper), keeping the actual algebraic
    /// degree at 2 across all constraints.  Stwo's lifted protocol
    /// enforces actual degree, not the declared bound, but we keep them
    /// aligned for clarity (matches Blake2b/Mul/DivRem/Cpu).
    const LOG_CONSTRAINT_DEGREE_BOUND: u32 = 1;

    type PreprocessedColumn = PreprocessedColumn;
    type MainColumn = Column;
    /// A 2-tuple — Range256 for byte ranges +
    ...
    fn add_constraints<E: EvalAtRow>(
        ...
        // Lives in `field_op_constraints` so RistrettoFixedBaseConsumerChip
        ...
        // ── Register-file inter-row binding ──
        //
        ...
        // Soundness note: the SubBorrow / FinalFormBorrow chains are
        // constrained to {0,1} only.  Individual byte arithmetic on
        // `intermediate − is_overflow·p` or `p − out − 1` relies on the
        // per-byte values being u8; a full pin would add a Range256
        // emission on those implicit byte expressions.
...
impl BuiltInProverComponent for RistrettoChip {
    ...
    fn generate_preprocessed_trace(&self, log_size: u32, _side_note: &SideNote) -> FinalizedTrace {
        ...
        for row in 0..num_rows {
            ...
            // Row index split into 2 LE bytes.  Limited
    ...
    fn generate_main_trace_immut_min(
        ...
        // Lay each FieldOpRow into its column slots.
