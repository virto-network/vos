...
>   * **INCREMENT (b) LANDED @ log 17 2026-06-23 (`child_full_measure`, commit
>     `6ea8c28`; standalone de-risk `935c8d4`+`27af2b5`, `recursion_deep_msponge.rs`):
>     the consumer rides the REAL trace-decommit m_sponge leaf rows.** Each m_sponge
>     row's 8 chunk leaves (sorted order) drain −1 keyed by `(0, route_s, cc_s)`:
>     `route_s` preproc sorted→flat (`flat = flat_offset + col_order[sorted_pos]`,
>     proven == the DEEP `column_index` into `queried_values.flatten()`), `cc_s`
>     witnessed (= `c_map[flat]`, forced to the producer's c by the logup balance).
>     **8 logup fractions/row → `finalize_logup_in_pairs` → 4 interaction cols** (the
>     in-pairs gen pairs entries (2b,2b+1) with combined `(n0·d1+n1·d0)/(d0·d1)`).
>     **KEY NEW MECHANISM: per-query SCATTER-ADD lanes** — the tree-major decommit
>     stream scatters a query's leaf rows (tree0 path-qi, then tree1 path-qi, …), so a
>     contiguous carry can't reach them; instead NQ=38 lane accumulators, each a
>     `[-1,0]` hold/advance col adding the row's witnessed `inc` only when its one-hot
>     `qsel_q` fires (wrap broken at logical row 0 via `lane_cont`), bound to the honest
>     `host_l` oracle on a bind row. Producer scaled to ALL batch-0 columns (one row
>     each, +N_QUERIES). `TreeData` gained `col_order` (the sort permutation); `MkRow`
>     gained `deep_qi`/`deep_flat`/`deep_valid` (set in `mk_resolve`). `build_inputs`
>     ASSERTS the identity preproc remap (`preprocessed_query_positions ==
>     query_positions` ⇐ all trees height = lifting_log_size ⇒ path index = query index)
>     AND that batch 0 covers every consumed column (the `c_map` `.expect`). preproc
>     6224→6281 (+57 DEEP cols); deg≤2; FRI tamper still rejects. **ADVERSARIAL REVIEW
>     (Workflow, 19 agents, 14 findings, 5 confirmed) found 2 real issues: (A, critical,
>     FIXED) the DEEP logup `claimed_sum` was never constrained to 0 — stwo's `verify`
>     only enforces `Σ(fractions)==claimed_sum`, so a free claimed_sum made the leaf↔c
>     logup VACUOUS (a prover dumps producer/consumer imbalance into it, decoupling cc
>     from c); fix = re-add the verify-side `deep_claimed==0` check (mirrors the
>     standalone gates), no re-prove; the self-certifying in-AIR boundary (`is_last_row·
>     last_cumsum==0`) lands with (c)'s re-prove. (B, latent-in-b, → (c)) the producer's
>     `deep_pow` (α^i) is a FREE witness (no α-power chain off `squeeze[2]`) and
>     `deep_zy` is held-but-unbound (not tied to the embed's OODS `oodsy`) ⇒ per-column
>     c is prover-chosen; HARMLESS in (b) (host_l is honest, no false statement) but
>     increment (c) MUST bind both before swapping host_l for the real first_layer.**
>   * **INCREMENT (c) — precise scope (mapped 2026-06-23):** the `eval[qi]==first_layer[qi]`
>     bind needs the **FULL multi-batch** eval — `first_layer_evals[qi]` is the DEEP
>     quotient = `Σ_b denom_inv[b]·(L[b][qi] − p.y·A[b] − B[b])` over ALL 11 batches, NOT
>     just batch 0. So (c) must: (1) per-batch lanes `L[b][qi]` (the consumer adds
>     `leaf·c[b]` for EACH batch a col is in — the multi-batch-per-leaf multiplicity;
>     most cols are batch-0-only); (2) producer derives `a,b,c` per (batch,col), latch
>     `A[b]=Σa`, `B[b]=Σb` (query-independent); (3) evalfin rows (per query) compute
>     `eval[qi]=Σ_b denom_inv[b]·(L[b][qi] − p[qi].y·A[b] − B[b])` — witness `pA=p.y·A`
>     and `num=L−pA−B` to keep deg 2, `eval=num·denom_inv` (deg 2), `denom_inv` from
>     latched z[b] + per-query `p[qi]` (the 4b CM31 denom); (4) the `eval_lat[qi]`
>     carrier (deep_full-proven) binds `eval[qi]` (evalfin row) to the FRI layer-0
>     `running = e0 + mux_fold` (== `first_layer_evals[qi]`) on the layer-0 fold row
>     (`fri.per_query` is query-major; add a per-FRI-fold query index + one-hot selector,
>     analog of `deep_qi`). PARITY: `p[qi].y` is per-query parity-dependent (odd queries
>     `p.y = −even.y`); NOT the bare fold twiddle. The assert gate CATCHES a wrong p.y
>     (eval ≠ first_layer ⇒ `eval_lat==eval` & `eval_lat==first_layer` inconsistent), so
>     only degree needs the prove. Also land Issue B (deep_pow↔α-chain, deep_zy↔oodsy)
>     + the in-AIR claimed_sum boundary. v's `mix_felts(sampled_values)` binding = (d).
