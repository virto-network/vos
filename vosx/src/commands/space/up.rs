...
pub fn run(args: Args) -> anyhow::Result<()> {
    ...
    if args.once {
        ...
        // chronos clock/randomness feed (see `ChronosFeeder`): a separate,
        // faster keepalive gate than the spawn-reconcile pass. The per-space
        // domain is the space id; the feeder holds its own cross-pass state and
        // the node's static VRF keypair. A node.key read failure disables the
        // feed rather than the whole daemon.
        let mut chronos_feeder = match ChronosFeeder::new(&data_dir, entry.id.as_bytes().to_vec()) {
            Ok(f) => Some(f),
            Err(e) => {
...
/// [`VOS_COMMON_ERA_MS`]. The actor's epoch is `SLOTS_PER_EPOCH` of these.
...
/// Drives the per-space `chronos` clock + v1 bias-resistance protocol from the
/// vosx daemon's periodic hook. Holds the node's static VRF keypair (derived
/// once from `node.key`) and the cross-pass feed state.
///
...
/// (soft-restarts) when committed entries are ahead of what it has applied
/// (`CommitStrategy::needs_sync_reload`), so the leader no longer replays its
/// whole log on every one of its own commits — the change that lets the chronos
/// clock stay live indefinitely instead of stalling. Reveals/enrols authenticate
/// by the VRF proof + the leader-pushed authorized set, NOT by the caller (a
/// chronos handler runs on the raft apply path, where the originating caller is
/// not preserved), so the follower's fire-and-forget cross-node write is bound
/// to its voter by the proof it carries, not by its connection identity.
struct ChronosFeeder {
...
impl ChronosFeeder {
    ...
    fn new(data_dir: &std::path::Path, domain: Vec<u8>) -> anyhow::Result<Self> {
        let key_bytes = std::fs::read(data_dir.join("node.key"))
            .map_err(|e| anyhow::anyhow!("chronos feeder: read node.key: {e}"))?;
        let keypair = libp2p::identity::Keypair::from_protobuf_encoding(&key_bytes)
            .map_err(|e| anyhow::anyhow!("chronos feeder: decode node.key: {e}"))?;
        ...
        let seed = vos::crypto::blake2b_hash(b"vos-chronos-vrf/v1", &[&key_bytes]);
    ...
    fn feed(&mut self, node: &mut VosNode, local_prefix: u16) {
        ...
        let is_leader = status
            .as_ref()
            .is_some_and(|s| s.role == vos::network::RaftRole::Leader);
        ...
        let leader_chronos = crate::commands::space::common::instance_service_id("chronos", lp);
    ...
    fn drive_clock(&mut self, node: &mut VosNode, chronos_id: ServiceId) {
        use chronos::ChronosRef;
        let chronos = ChronosRef::at(chronos_id);
        if self.clock.is_none() {
            if vos::block_on(chronos.init(&mut &*node, self.domain.clone())).is_err() {
                return;
            }
            match vos::block_on(chronos.now(&mut &*node)) {
        ...
        match vos::block_on(chronos.advance(&mut &*node, proposed, entropy.to_vec())) {
    ...
    fn enrol_self(
        ...
        if let Ok(committee) = vos::block_on(local.committee(&mut &*node))
        ...
        let payload = {
            ...
            let _ = vos::block_on(chronos.enrol_voter(
                &mut cap,
                self.local_peer.clone(),
                self.vrf_pk_bytes.clone(),
            ));
    ...
    fn post_reveals(
        ...
        use chronos::ChronosRef;
        let local = ChronosRef::at(chronos_id);
        let Ok(open) = vos::block_on(local.open_rounds(&mut &*node)) else {
            return;
        };
        // Forget rounds that have folded (left the open set), so the set stays
        // small and a re-opened index can't be wrongly skipped.
        let open_set: std::collections::HashSet<u64> = open.iter().map(|o| o.round).collect();
        ...
        for o in &open {
            ...
            if is_leader {
                let chronos = ChronosRef::at(chronos_id);
                if vos::block_on(chronos.reveal(
                    &mut &*node,
                    self.local_peer.clone(),
                    o.round,
                ...
                let payload = {
                    let mut cap = CaptureInvoker::default();
                    let chronos = ChronosRef::at(leader_chronos);
                    let _ = vos::block_on(chronos.reveal(
                        &mut cap,
                        self.local_peer.clone(),
                        o.round,
                        proof.to_bytes().to_vec(),
...
/// An [`Invoker`](vos::actors::client::Invoker) that *captures* the encoded call
/// instead of dispatching it, so the feeder can hand a follower's enrol/reveal
/// to `net.send_invoke` as a fire-and-forget cross-node write. The returned
/// reply is a placeholder (the caller ignores it and reads `payload`).
#[derive(Default)]
struct CaptureInvoker {
    payload: Vec<u8>,
}

impl vos::actors::client::Invoker for CaptureInvoker {
    fn invoke(
        &mut self,
        _target: ServiceId,
        payload: Vec<u8>,
    ) -> impl std::future::Future<
        Output = Result<vos::actors::value::Value, vos::actors::client::ClientError>,
    > + '_ {
        self.payload = payload;
        std::future::ready(Ok(vos::actors::value::Value::Unit))
    }
}
