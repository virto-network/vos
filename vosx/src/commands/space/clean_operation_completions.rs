...
use vos::agent::clean_bootstrap::{
    MAX_NATIVE_OPERATION_COMPLETION_BYTES, native_operation_completion_invocations,
};
...
impl CleanNativeAuthorityOperationCompletions {
    ...
    fn open(
        ...
        let store = Self {
            file: ExactFileStore::new(root, StoreRole::OperationCompletions),
            authority,
        };
        store.load()?;
        Ok(store)
    ...
    fn encode(&self, records: &[Vec<u8>]) -> Result<Vec<u8>, CleanFileStoreError> {
        ...
        let mut bytes = b"NCI1".to_vec();
        bytes.extend_from_slice(&self.scope().0);
        bytes.extend_from_slice(&(records.len() as u32).to_le_bytes());
        for record in records {
            if record.len() > MAX_NATIVE_OPERATION_COMPLETION_BYTES {
                return Err(CleanFileStoreError::Oversized);
            }
    ...
    fn decode(&self, bytes: &[u8]) -> Result<Vec<Vec<u8>>, CleanFileStoreError> {
        ...
        if bytes.len() < 40 || &bytes[..4] != b"NCI1" || bytes[4..36] != self.scope().0 {
        ...
        for _ in 0..count {
            ...
            if size > MAX_NATIVE_OPERATION_COMPLETION_BYTES {
                return Err(CleanFileStoreError::Oversized);
            }
            let record = bytes
    ...
    pub(crate) fn load(&self) -> Result<Vec<Vec<u8>>, CleanFileStoreError> {
        ...
        let canonical = self.file.read_optional(self.file.file(), MAX_IMAGE)?;
        let staged = self.file.read_optional(self.file.stage_file(), MAX_IMAGE)?;
        ...
        let image = self.file.reconcile(MAX_IMAGE)?;
        match image {
            Some(image) => {
